...
# 23 "io_info_args_rt.h"
int io_info_args_rt(pe_t * pe, rt_t * rt, const char * name, io_info_rw_enum_t rw, io_info_args_t * args); 
# 25
int io_info_args_rt_iogrid(pe_t * pe, rt_t * rt, const char * key, int  iogrid[3]); 
# 18 "physics.h"
...
# 129
io_info_t *io_rho = (__null); 
# 131
(pe) ? static_cast< void>(0) : __assert_fail("pe", "distribution_rt.c", 131, __PRETTY_FUNCTION__); 
# 132
(cs) ? static_cast< void>(0) : __assert_fail("cs", "distribution_rt.c", 132, __PRETTY_FUNCTION__); 
# 133
(rt) ? static_cast< void>(0) : __assert_fail("rt", "distribution_rt.c", 133, __PRETTY_FUNCTION__); 
# 134
(lb) ? static_cast< void>(0) : __assert_fail("lb", "distribution_rt.c", 134, __PRETTY_FUNCTION__); 
# 136
nreduced = 0; 
# 137
rt_string_parameter(rt, "reduced_halo", string, 4096); 
# 138
...
# 147
rt_string_parameter(rt, "distribution_io_format_input", string, 4096); 
# 153
if ((DATA_MODEL_SOA) == (DATA_MODEL_SOA)) { memory = 'R'; }  
# 154
lb_ndist(lb, &ndist); 
# 156
pe_info(pe, "\n"); 
...
# 199
rt_string_parameter(rt, "rho_io_format", string, 4096); 
# 201
((param.grid)[X]) = (io_grid[X]); 
# 202
((param.grid)[Y]) = (io_grid[Y]); 
...
# 238
int lb_rt_initial_conditions(pe_t *pe, rt_t *rt, lb_t *lb, physics_t *
# 239
phys) { 
# 241
char key[4096] = ""; 
...
# 245
(pe) ? static_cast< void>(0) : __assert_fail("pe", "distribution_rt.c", 245, __PRETTY_FUNCTION__); 
# 246
(rt) ? static_cast< void>(0) : __assert_fail("rt", "distribution_rt.c", 246, __PRETTY_FUNCTION__); 
# 247
(lb) ? static_cast< void>(0) : __assert_fail("lb", "distribution_rt.c", 247, __PRETTY_FUNCTION__); 
# 248
(phys) ? static_cast< void>(0) : __assert_fail("phys", "distribution_rt.c", 248, __PRETTY_FUNCTION__); 
# 250
physics_rho0(phys, &rho0); 
# 254
lb_init_rest_f(lb, rho0); 
# 256
rt_string_parameter(rt, "distribution_initialisation", key, 4096); 
# 258
...
double rho = (1.0); 
# 328
double u0 = (0.04000000000000000083); 
# 329
double delta = (0.05000000000000000278); 
# 330
double kappa = (80.0); 
# 331
double u[3]; 
...
# 338
(lb) ? static_cast< void>(0) : __assert_fail("lb", "distribution_rt.c", 338, __PRETTY_FUNCTION__); 
# 340
...
const double pi = (3.141592653589793116); 
# 402
(lb) ? static_cast< void>(0) : __assert_fail("lb", "distribution_rt.c", 402, __PRETTY_FUNCTION__); 
# 404
cs_nlocal(lb->cs, nlocal); 
# 405
cs_nlocal_offset(lb->cs, noffset); 
...
int nlocal[3]; 
# 449
(lb) ? static_cast< void>(0) : __assert_fail("lb", "distribution_rt.c", 449, __PRETTY_FUNCTION__); 
# 451
cs_nlocal(lb->cs, nlocal); 
# 453
for (ic = 1; ic <= (nlocal[X]); ic++) { 
...
double x, y, z; 
# 489
(lb) ? static_cast< void>(0) : __assert_fail("lb", "distribution_rt.c", 489, __PRETTY_FUNCTION__); 
# 491
cs_nlocal(lb->cs, nlocal); 
# 492
cs_nlocal_offset(lb->cs, noffset); 
...
# 1 "distribution_rt.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__23_distribution_rt_cpp1_ii_cd2381ce
# 1 "distribution_rt.cudafe1.stub.c"
#include "distribution_rt.cudafe1.stub.c"
# 1 "distribution_rt.cudafe1.stub.c"
