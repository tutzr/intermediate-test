...
# 81 "stats_sigma.c"
int stats_sigma_create(pe_t *pe, cs_t *cs, fe_symm_t *fe, field_t *phi, stats_sigma_t **
# 82
pobj) { 
# 84
stats_sigma_t *obj = (__null); 
# 85
int ntotal[3]; 
# 86
double xi0; 
...
fe_symm_param_t param; 
# 92
(pe) ? static_cast< void>(0) : __assert_fail("pe", "stats_sigma.c", 92, __PRETTY_FUNCTION__); 
# 93
(cs) ? static_cast< void>(0) : __assert_fail("cs", "stats_sigma.c", 93, __PRETTY_FUNCTION__); 
# 94
(pobj) ? static_cast< void>(0) : __assert_fail("pobj", "stats_sigma.c", 94, __PRETTY_FUNCTION__); 
# 96
obj = ((stats_sigma_t *)calloc(1, sizeof(stats_sigma_t))); 
# 97
(obj) ? static_cast< void>(0) : __assert_fail("obj", "stats_sigma.c", 97, __PRETTY_FUNCTION__); 
# 98
if (obj == (__null)) { pe_fatal(pe, "calloc(stats_sigma_t) failed\n"); }  
# 100
(obj->pe) = pe; 
# 101
(obj->cs) = cs; 
...
# 143
tdiff = (((((2.0) * xi0) * (2.0)) * xi0) / ((-mobility) / (param.a))); 
# 145
pe_info(pe, "\n"); 
# 146
...
int stats_sigma_free(stats_sigma_t *stat) { 
# 166
(stat) ? static_cast< void>(0) : __assert_fail("stat", "stats_sigma.c", 166, __PRETTY_FUNCTION__); 
# 168
free(stat); 
# 170
return 0; 
...
# 193
pe_info(stat->pe, "[sigma] %14d %14.7e %14.7e %14.7e\n", ntime, (stat->drop).radius, (stat->drop).xi0, (stat->drop).sigma); 
# 195
}  
# 197
return 0; 
...
MPI_Comm comm; 
# 220
(stat) ? static_cast< void>(0) : __assert_fail("stat", "stats_sigma.c", 220, __PRETTY_FUNCTION__); 
# 222
...
MPI_Comm comm; 
# 281
(stat) ? static_cast< void>(0) : __assert_fail("stat", "stats_sigma.c", 281, __PRETTY_FUNCTION__); 
# 283
cs_nlocal(stat->cs, nlocal); 
# 284
cs_nlocal_offset(stat->cs, noffset); 
...
# 314
(phi0 != phi1) ? static_cast< void>(0) : __assert_fail("phi0 != phi1", "stats_sigma.c", 314, __PRETTY_FUNCTION__); 
# 315
...
int nfit, nbestfit; 
# 363
int nphi[128], nphi_local[128]; 
# 364
double phir[128], phir_local[128]; 
# 366
double rmin; 
# 367
double rmax; 
...
double r0, dr, r[3]; 
# 370
double cost, costmin, phi; 
# 371
double xi0, xi0fit; 
# 373
MPI_Comm comm; 
# 375
(stat) ? static_cast< void>(0) : __assert_fail("stat", "stats_sigma.c", 375, __PRETTY_FUNCTION__); 
# 377
cs_nlocal(stat->cs, nlocal); 
# 378
cs_nlocal_offset(stat->cs, noffset); 
...
# 385
rmin = (((stat->drop).radius) - ((10.0) * xi0)); 
# 386
rmax = (((stat->drop).radius) + ((10.0) * xi0)); 
# 387
dr = ((rmax - rmin) / (128)); 
# 389
for (n = 0; n < 128; n++) { 
...
# 422
MPI_Reduce(phir_local, phir, 128, MPI_DOUBLE, MPI_SUM, 0, comm); 
# 423
MPI_Reduce(nphi_local, nphi, 128, MPI_INT, MPI_SUM, 0, comm); 
# 425
for (n = 0; n < 128; n++) { 
# 426
...
nbestfit = (-1); 
# 434
costmin = ((double)(1.7976931348623157081e+308L)); 
# 436
for (nfit = 0; nfit < 2000; nfit++) { 
# 437
cost = (0.0); 
# 438
xi0fit = ((((2.0) * (nfit + 1)) * xi0) / (2000)); 
# 439
for (n = 0; n < 128; n++) { 
# 440
r0 = (rmin + ((n + (0.5)) * dr)); 
...
# 452
(nbestfit > 0) ? static_cast< void>(0) : __assert_fail("nbestfit > 0", "stats_sigma.c", 452, __PRETTY_FUNCTION__); 
# 453
xi0fit = ((((2.0) * (nbestfit + 1)) * xi0) / (2000)); 
# 454
...
double fe; 
# 477
double fmin, fmin_local; 
# 478
double excess, excess_local; 
# 479
const double pi = (3.141592653589793116); 
# 481
MPI_Comm comm; 
# 483
(stat) ? static_cast< void>(0) : __assert_fail("stat", "stats_sigma.c", 483, __PRETTY_FUNCTION__); 
# 485
cs_nlocal(stat->cs, nlocal); 
# 486
pe_mpi_comm(stat->pe, &comm); 
# 490
fmin_local = ((3.402823466e+38F)); 
# 492
for (ic = 1; ic <= (nlocal[X]); ic++) { 
# 493
for (jc = 1; jc <= (nlocal[Y]); jc++) { 
...
# 1 "stats_sigma.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__19_stats_sigma_cpp1_ii_40acddcd
# 1 "stats_sigma.cudafe1.stub.c"
#include "stats_sigma.cudafe1.stub.c"
# 1 "stats_sigma.cudafe1.stub.c"
