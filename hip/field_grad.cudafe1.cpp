...
# 89
tdpMemcpy(&((obj->target)->field), &((obj->field)->target), sizeof(field_t *), hipMemcpyHostToDevice); 
# 91
}  
# 93
if ((obj->level) >= 2) { 
...
# 105
tdpMemcpy(&((obj->target)->grad), &tmp, sizeof(double *), hipMemcpyHostToDevice); 
# 108
tdpMalloc((void **)(&tmp), ((obj->nf) * nsites) * sizeof(double)); 
# 109
tdpMemcpy(&((obj->target)->delsq), &tmp, sizeof(double *), hipMemcpyHostToDevice); 
# 111
}  
# 112
}  
...
# 120
tdpMemcpy(&((obj->target)->d_ab), &tmp, sizeof(double *), hipMemcpyHostToDevice); 
# 122
}  
# 123
}  
...
# 133
tdpMemcpy(&((obj->target)->grad_delsq), &tmp, sizeof(double *), hipMemcpyHostToDevice); 
# 136
tdpMalloc((void **)(&tmp), ((obj->nf) * nsites) * sizeof(double)); 
# 137
tdpMemcpy(&((obj->target)->delsq_delsq), &tmp, sizeof(double *), hipMemcpyHostToDevice); 
# 139
}  
# 140
}  
...
tdpMemcpy(&((obj->target)->nsite), &(obj->nsite), sizeof(int), hipMemcpyHostToDevice); 
# 178
tdpMemcpy(&tmp, &((obj->target)->grad), sizeof(double *), hipMemcpyDeviceToHost); 
# 180
tdpMemcpy(tmp, obj->grad, (3) * nsz, hipMemcpyHostToDevice); 
# 181
tdpMemcpy(&tmp, &((obj->target)->delsq), sizeof(double *), hipMemcpyDeviceToHost); 
# 183
tdpMemcpy(tmp, obj->delsq, nsz, hipMemcpyHostToDevice); 
# 184
break; 
# 185
case hipMemcpyDeviceToHost:  
...
tdpMemcpy(&tmp, &((obj->target)->grad), sizeof(double *), hipMemcpyDeviceToHost); 
# 188
tdpMemcpy(obj->grad, tmp, (3) * nsz, hipMemcpyDeviceToHost); 
# 189
tdpMemcpy(&tmp, &((obj->target)->delsq), sizeof(double *), hipMemcpyDeviceToHost); 
# 191
tdpMemcpy(obj->delsq, tmp, nsz, hipMemcpyDeviceToHost); 
# 192
break; 
# 193
default:  
...
tdpFree(tmp); 
# 260
tdpMemcpy(&tmp, &((obj->target)->d_ab), sizeof(double *), hipMemcpyDeviceToHost); 
# 262
if (tmp) { tdpFree(tmp); }  
# 263
tdpMemcpy(&tmp, &((obj->target)->grad_delsq), sizeof(double *), hipMemcpyDeviceToHost); 
# 265
if (tmp) { tdpFree(tmp); }  
# 266
tdpMemcpy(&tmp, &((obj->target)->delsq_delsq), sizeof(double *), hipMemcpyDeviceToHost); 
# 268
if (tmp) { tdpFree(tmp); }  
# 270
tdpFree(obj->target); 
