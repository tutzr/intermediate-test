...
# 339
if (sum < ((double)(2.2250738585072013831e-308L))) { return 0; }  
# 341
if (iterate < 4) { 
# 342
tresh = (((0.2000000000000000111) * sum) / (3 * 3)); } else { 
# 344
tresh = (0.0); }  
# 346
for (ia = 0; ia < 2; ia++) { 
...
# 351
if ((iterate > 4) && ((fabs(vals[ia]) + g) == fabs(vals[ia])) && ((fabs(vals[ib]) + g) == fabs(vals[ib]))) 
# 352
{ 
# 353
((a[ia])[ib]) = (0.0); 
...
# 641
wmin = (((double)(2.2204460492503130808e-16L)) * wmax); 
# 645
k = 0; 
# 647
for (j = 0; j < n; j++) { 
...
extern intmax_t imaxabs(intmax_t __n) throw() __attribute((const)); 
# 294
extern imaxdiv_t imaxdiv(intmax_t __numer, intmax_t __denom) throw()
# 295
 __attribute((const)); 
# 298
extern intmax_t strtoimax(const char *__restrict__ __nptr, char **__restrict__ __endptr, int __base) throw(); 
# 302
extern uintmax_t strtoumax(const char *__restrict__ __nptr, char **__restrict__ __endptr, int __base) throw(); 
# 306
extern intmax_t wcstoimax(const wchar_t *__restrict__ __nptr, wchar_t **__restrict__ __endptr, int __base) throw(); 
# 311
extern uintmax_t wcstoumax(const wchar_t *__restrict__ __nptr, wchar_t **__restrict__ __endptr, int __base) throw(); 
# 433 "/usr/include/inttypes.h" 3
}
# 1200 "util.c"
static long util_ranlcg_multiply(long a, long s, long c, long m); 
# 1223 "util.c"
int util_ranlcg_reap_gaussian(int *state, double r[2]) { 
# 1225
double ranu[2]; 
...
# 1 "util.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__12_util_cpp1_ii_429d0192
# 1 "util.cudafe1.stub.c"
#include "util.cudafe1.stub.c"
# 1 "util.cudafe1.stub.c"
