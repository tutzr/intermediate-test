...
# 38
int beris_edw_create(pe_t * pe, cs_t * cs, lees_edw_t * le, beris_edw_t ** pobj); 
# 40
int beris_edw_free(beris_edw_t * be); 
# 41
int beris_edw_memcpy(beris_edw_t * be, int flag); 
# 42
int beris_edw_param_set(beris_edw_t * be, beris_edw_param_t * values); 
# 43
int beris_edw_param_commit(beris_edw_t * be); 
# 45
int beris_edw_update(beris_edw_t * be, fe_t * fe, field_t * fq, field_grad_t * fq_grad, hydro_t * hydro, colloids_info_t * cinfo, map_t * map, noise_t * noise); 
# 50
int beris_edw_tmatrix(double  t[3][3][5]); 
# 22 "advection_s.h"
struct advflux_s { 
# 24
int nf; 
...
# 62 "blue_phase_beris_edwards.c"
int beris_edw_update_driver(beris_edw_t * be, field_t * fq, field_grad_t * fq_grad, hydro_t * hydro, map_t * map, noise_t * noise); 
# 66
int beris_edw_fix_swd(beris_edw_t * be, colloids_info_t * cinfo, hydro_t * hydro, map_t * map); 
# 68
int beris_edw_update_host(beris_edw_t * be, fe_t * fe, field_t * fq, hydro_t * hydro, advflux_t * flux, map_t * map, noise_t * noise); 
# 71
int beris_edw_h_driver(beris_edw_t * be, fe_t * fe); 
# 74
void beris_edw_h_kernel_v(kernel_ctxt_t * ktx, beris_edw_t * be, fe_t * fe); 
# 76
void beris_edw_kernel_v(kernel_ctxt_t * ktx, beris_edw_t * be, field_t * fq, field_grad_t * fqgrad, hydro_t * hydro, advflux_t * flux, map_t * map, noise_t * noise); 
# 81
void beris_edw_fix_swd_kernel(kernel_ctxt_t * ktx, colloids_info_t * cinfo, hydro_t * hydro, map_t * map, int noffsetx, int noffsety, int noffsetz); 
# 85
struct beris_edw_s { 
# 86
beris_edw_param_t *param; 
# 87
cs_t *cs; 
# 88
lees_edw_t *le; 
...
# 96
static beris_edw_param_t static_param; 
# 107 "blue_phase_beris_edwards.c"
int beris_edw_create(pe_t *pe, cs_t *cs, lees_edw_t *le, beris_edw_t **
# 108
pobj) { 
# 110
int ndevice; 
# 111
advflux_t *flx = (__null); 
# 112
beris_edw_t *obj = (__null); 
# 114
(pe) ? static_cast< void>(0) : __assert_fail("pe", "blue_phase_beris_edwards.c", 114, __PRETTY_FUNCTION__); 
# 115
(cs) ? static_cast< void>(0) : __assert_fail("cs", "blue_phase_beris_edwards.c", 115, __PRETTY_FUNCTION__); 
# 116
(le) ? static_cast< void>(0) : __assert_fail("le", "blue_phase_beris_edwards.c", 116, __PRETTY_FUNCTION__); 
# 117
(pobj) ? static_cast< void>(0) : __assert_fail("pobj", "blue_phase_beris_edwards.c", 117, __PRETTY_FUNCTION__); 
# 119
obj = ((beris_edw_t *)calloc(1, sizeof(beris_edw_t))); 
# 120
(obj) ? static_cast< void>(0) : __assert_fail("obj", "blue_phase_beris_edwards.c", 120, __PRETTY_FUNCTION__); 
# 121
if (obj == (__null)) { pe_fatal(pe, "calloc(beris_edw) failed\n"); }  
# 123
(obj->param) = ((beris_edw_param_t *)calloc(1, sizeof(beris_edw_param_t))); 
# 124
if ((obj->param) == (__null)) { pe_fatal(pe, "calloc(beris_edw_param_t) failed\n"); }  
# 126
advflux_le_create(pe, cs, le, 5, &flx); 
# 127
(flx) ? static_cast< void>(0) : __assert_fail("flx", "blue_phase_beris_edwards.c", 127, __PRETTY_FUNCTION__); 
# 129
lees_edw_nsites(le, &(obj->nall)); 
# 130
(obj->h) = ((double *)calloc((obj->nall) * 5, sizeof(double))); 
# 131
(obj->h) ? static_cast< void>(0) : __assert_fail("obj->h", "blue_phase_beris_edwards.c", 131, __PRETTY_FUNCTION__); 
# 133
(obj->cs) = cs; 
# 134
(obj->le) = le; 
...
# 151
{ tdpErrorHandler(tdpMalloc((void **)(&(obj->target)), sizeof(beris_edw_t)), "blue_phase_beris_edwards.c", 151, 1); } ; 
# 152
{ tdpErrorHandler(tdpMemset(obj->target, 0, sizeof(beris_edw_t)), "blue_phase_beris_edwards.c", 152, 1); } ; 
# 153
{ tdpErrorHandler(hipGetSymbolAddress((void **)(&tmp), &static_param), "blue_phase_beris_edwards.c", 153, 1); } ; 
# 154
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->param), &tmp, sizeof(beris_edw_param_t *), hipMemcpyHostToDevice), "blue_phase_beris_edwards.c", 155, 1); 
# 154 "blue_phase_beris_edwards.c"
} 
# 155
; 
# 157
...
# 158
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->le), &letarget, sizeof(lees_edw_t *), hipMemcpyHostToDevice), "blue_phase_beris_edwards.c", 159, 1); 
# 158 "blue_phase_beris_edwards.c"
} 
# 159
; 
# 160
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->flux), &(flx->target), sizeof(advflux_t *), hipMemcpyHostToDevice), "blue_phase_beris_edwards.c", 161, 1); 
# 160 "blue_phase_beris_edwards.c"
} 
# 161
; 
# 163
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->nall), &(obj->nall), sizeof(int), hipMemcpyHostToDevice), "blue_phase_beris_edwards.c", 164, 1); 
# 163 "blue_phase_beris_edwards.c"
} 
# 164
; 
# 165
{ tdpErrorHandler(tdpMalloc((void **)(&htmp), ((obj->nall) * 5) * sizeof(double)), "blue_phase_beris_edwards.c", 165, 1); } ; 
# 166
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->h), &htmp, sizeof(double *), hipMemcpyHostToDevice), "blue_phase_beris_edwards.c", 167, 1); 
# 166 "blue_phase_beris_edwards.c"
} 
# 167
; 
# 168
...
int ndevice; 
# 185
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 185, __PRETTY_FUNCTION__); 
# 187
tdpGetDeviceCount(&ndevice); 
# 189
if (ndevice > 0) { 
...
double *htmp; 
# 192
{ tdpErrorHandler(tdpMemcpy(&htmp, &((be->target)->h), sizeof(double *), hipMemcpyDeviceToHost), "blue_phase_beris_edwards.c", 193, 1); 
# 192 "blue_phase_beris_edwards.c"
} 
# 193
; 
# 194
{ tdpErrorHandler(tdpFree(htmp), "blue_phase_beris_edwards.c", 194, 1); } ; 
# 195
{ tdpErrorHandler(tdpFree(be->target), "blue_phase_beris_edwards.c", 195, 1); } ; 
# 196
}  
# 198
...
# 217
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 217, __PRETTY_FUNCTION__); 
# 219
physics_ref(&phys); 
# 221
physics_kt(phys, &kt); 
...
# 224
{ tdpErrorHandler(hipMemcpyToSymbol(&static_param, be->param, sizeof(beris_edw_param_t), 0, hipMemcpyHostToDevice), "blue_phase_beris_edwards.c", 225, 1); 
# 224 "blue_phase_beris_edwards.c"
} 
# 225
; 
# 227
...
# 236
int beris_edw_param_set(beris_edw_t *be, beris_edw_param_t *vals) { 
# 238
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 238, __PRETTY_FUNCTION__); 
# 239
(vals) ? static_cast< void>(0) : __assert_fail("vals", "blue_phase_beris_edwards.c", 239, __PRETTY_FUNCTION__); 
# 241
(*(be->param)) = (*vals); 
# 243
return 0; 
...
# 260 "blue_phase_beris_edwards.c"
int beris_edw_update(beris_edw_t *be, fe_t *
# 261
fe, field_t *
# 262
fq, field_grad_t *
...
int nf; 
# 270
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 270, __PRETTY_FUNCTION__); 
# 271
(fq) ? static_cast< void>(0) : __assert_fail("fq", "blue_phase_beris_edwards.c", 271, __PRETTY_FUNCTION__); 
# 272
(map) ? static_cast< void>(0) : __assert_fail("map", "blue_phase_beris_edwards.c", 272, __PRETTY_FUNCTION__); 
# 274
field_nf(fq, &nf); 
# 275
(nf == 5) ? static_cast< void>(0) : __assert_fail("nf == 5", "blue_phase_beris_edwards.c", 275, __PRETTY_FUNCTION__); 
# 277
if (hydro) { 
# 278
...
# 307 "blue_phase_beris_edwards.c"
int beris_edw_update_host(beris_edw_t *be, fe_t *fe, field_t *fq, hydro_t *
# 308
hydro, advflux_t *flux, map_t *
# 309
map, noise_t *noise) { 
# 310
int ic, jc, kc; 
...
# 333
const int8_t d_[3][3] = {{(1), (0), (0)}, {(0), (1), (0)}, {(0), (0), (1)}}; 
# 335
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 335, __PRETTY_FUNCTION__); 
# 336
(fe) ? static_cast< void>(0) : __assert_fail("fe", "blue_phase_beris_edwards.c", 336, __PRETTY_FUNCTION__); 
# 337
((fe->func)->htensor) ? static_cast< void>(0) : __assert_fail("fe->func->htensor", "blue_phase_beris_edwards.c", 337, __PRETTY_FUNCTION__); 
# 338
(fq) ? static_cast< void>(0) : __assert_fail("fq", "blue_phase_beris_edwards.c", 338, __PRETTY_FUNCTION__); 
# 339
(flux) ? static_cast< void>(0) : __assert_fail("flux", "blue_phase_beris_edwards.c", 339, __PRETTY_FUNCTION__); 
# 340
(map) ? static_cast< void>(0) : __assert_fail("map", "blue_phase_beris_edwards.c", 340, __PRETTY_FUNCTION__); 
# 342
xi = ((be->param)->xi); 
# 343
...
if (noise_on) { 
# 358
(0) ? static_cast< void>(0) : __assert_fail("0", "blue_phase_beris_edwards.c", 358, __PRETTY_FUNCTION__); 
# 359
...
for (id = 0; id < 3; id++) { 
# 396
((s[ia])[ib]) += ((((xi * ((d[ia])[id])) + ((omega[ia])[id])) * (((q[id])[ib]) + (r3 * ((d_[id])[ib])))) + ((((q[ia])[id]) + (r3 * ((d_[ia])[id]))) * ((xi * ((d[id])[ib])) - ((omega[id])[ib])))); 
# 399
}  
# 400
}  
...
if (noise_on) { 
# 407
noise_reap_n(noise, index, 5, chi); 
# 408
for (id = 0; id < 5; id++) { 
# 409
(chi[id]) = (var * (chi[id])); 
...
# 427
((q[X])[X]) += (dt * ((((((((((s[X])[X]) + (gamma * ((h[X])[X]))) + ((chi_qab[X])[X])) - ((flux->fe)[reverse_addr_rank1_assert(428, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XX)])) + ((flux->fw)[reverse_addr_rank1_assert(429, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XX)])) - ((flux->fy)[reverse_addr_rank1_assert(430, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XX)])) + ((flux->fy)[reverse_addr_rank1_assert(431, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj, XX)])) - ((flux->fz)[reverse_addr_rank1_assert(432, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XX)])) + ((flux->fz)[reverse_addr_rank1_assert(433, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk, XX)]))); 
# 435
((q[X])[Y]) += (dt * ((((((((((s[X])[Y]) + (gamma * ((h[X])[Y]))) + ((chi_qab[X])[Y])) - ((flux->fe)[reverse_addr_rank1_assert(436, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XY)])) + ((flux->fw)[reverse_addr_rank1_assert(437, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XY)])) - ((flux->fy)[reverse_addr_rank1_assert(438, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XY)])) + ((flux->fy)[reverse_addr_rank1_assert(439, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj, XY)])) - ((flux->fz)[reverse_addr_rank1_assert(440, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XY)])) + ((flux->fz)[reverse_addr_rank1_assert(441, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk, XY)]))); 
# 443
((q[X])[Z]) += (dt * ((((((((((s[X])[Z]) + (gamma * ((h[X])[Z]))) + ((chi_qab[X])[Z])) - ((flux->fe)[reverse_addr_rank1_assert(444, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XZ)])) + ((flux->fw)[reverse_addr_rank1_assert(445, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XZ)])) - ((flux->fy)[reverse_addr_rank1_assert(446, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XZ)])) + ((flux->fy)[reverse_addr_rank1_assert(447, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj, XZ)])) - ((flux->fz)[reverse_addr_rank1_assert(448, "blue_phase_beris_edwards.c", flux->nsite, 5, index, XZ)])) + ((flux->fz)[reverse_addr_rank1_assert(449, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk, XZ)]))); 
# 451
((q[Y])[Y]) += (dt * ((((((((((s[Y])[Y]) + (gamma * ((h[Y])[Y]))) + ((chi_qab[Y])[Y])) - ((flux->fe)[reverse_addr_rank1_assert(452, "blue_phase_beris_edwards.c", flux->nsite, 5, index, YY)])) + ((flux->fw)[reverse_addr_rank1_assert(453, "blue_phase_beris_edwards.c", flux->nsite, 5, index, YY)])) - ((flux->fy)[reverse_addr_rank1_assert(454, "blue_phase_beris_edwards.c", flux->nsite, 5, index, YY)])) + ((flux->fy)[reverse_addr_rank1_assert(455, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj, YY)])) - ((flux->fz)[reverse_addr_rank1_assert(456, "blue_phase_beris_edwards.c", flux->nsite, 5, index, YY)])) + ((flux->fz)[reverse_addr_rank1_assert(457, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk, YY)]))); 
# 459
((q[Y])[Z]) += (dt * ((((((((((s[Y])[Z]) + (gamma * ((h[Y])[Z]))) + ((chi_qab[Y])[Z])) - ((flux->fe)[reverse_addr_rank1_assert(460, "blue_phase_beris_edwards.c", flux->nsite, 5, index, YZ)])) + ((flux->fw)[reverse_addr_rank1_assert(461, "blue_phase_beris_edwards.c", flux->nsite, 5, index, YZ)])) - ((flux->fy)[reverse_addr_rank1_assert(462, "blue_phase_beris_edwards.c", flux->nsite, 5, index, YZ)])) + ((flux->fy)[reverse_addr_rank1_assert(463, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj, YZ)])) - ((flux->fz)[reverse_addr_rank1_assert(464, "blue_phase_beris_edwards.c", flux->nsite, 5, index, YZ)])) + ((flux->fz)[reverse_addr_rank1_assert(465, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk, YZ)]))); 
# 467
...
# 489 "blue_phase_beris_edwards.c"
int beris_edw_update_driver(beris_edw_t *be, field_t *
# 490
fq, field_grad_t *
# 491
fq_grad, hydro_t *
...
# 501
hydro_t *hydrotarget = (__null); 
# 502
noise_t *noisetarget = (__null); 
# 504
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 504, __PRETTY_FUNCTION__); 
# 505
(fq) ? static_cast< void>(0) : __assert_fail("fq", "blue_phase_beris_edwards.c", 505, __PRETTY_FUNCTION__); 
# 506
(map) ? static_cast< void>(0) : __assert_fail("map", "blue_phase_beris_edwards.c", 506, __PRETTY_FUNCTION__); 
# 508
...
# 514
kernel_ctxt_create(be->cs, 1, limits, &ctxt); 
# 515
...
# 526
do { (__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : beris_edw_kernel_v(ctxt->target, be->target, fq->target, fq_grad->target, hydrotarget, (be->flux)->target, map->target, noisetarget); } while (0); 
# 528
; 
# 530
{ tdpErrorHandler(tdpPeekAtLastError(), "blue_phase_beris_edwards.c", 530, 1); } ; 
# 531
{ tdpErrorHandler(tdpDeviceSynchronize(), "blue_phase_beris_edwards.c", 531, 1); } ; 
# 533
...
# 547
void beris_edw_kernel_v(kernel_ctxt_t *ktx, beris_edw_t *be, field_t *
# 548
fq, field_grad_t *fqgrad, hydro_t *
# 549
hydro, advflux_t *flux, map_t *
# 550
map, noise_t *noise) ;
...
int kindex; 
# 553
__attribute__((unused)) int kiterations; 
# 555
const double dt = (1.0); 
# 556
const double r3 = ((1.0) / (3.0)); 
...
const int8_t d_[3][3] = {{(1), (0), (0)}, {(0), (1), (0)}, {(0), (0), (1)}}; 
# 559
(ktx) ? static_cast< void>(0) : __assert_fail("ktx", "blue_phase_beris_edwards.c", 559, __PRETTY_FUNCTION__); 
# 560
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 560, __PRETTY_FUNCTION__); 
# 561
(fq) ? static_cast< void>(0) : __assert_fail("fq", "blue_phase_beris_edwards.c", 561, __PRETTY_FUNCTION__); 
# 562
(fqgrad) ? static_cast< void>(0) : __assert_fail("fqgrad", "blue_phase_beris_edwards.c", 562, __PRETTY_FUNCTION__); 
# 563
(flux) ? static_cast< void>(0) : __assert_fail("flux", "blue_phase_beris_edwards.c", 563, __PRETTY_FUNCTION__); 
# 564
(map) ? static_cast< void>(0) : __assert_fail("map", "blue_phase_beris_edwards.c", 564, __PRETTY_FUNCTION__); 
# 566
...
for (ib = 0; ib < 3; ib++) { 
# 595
for (iv = 0; iv < 1; iv++) { (((s[ia])[ib])[iv]) = (0.0); }  
# 596
for (iv = 0; iv < 1; iv++) { (((chi_qab[ia])[ib])[iv]) = (0.0); }  
# 597
}  
# 598
}  
...
# 610
for (iv = 0; iv < 1; iv++) { (((q[X])[X])[iv]) = ((fq->data)[reverse_addr_rank1_assert(610, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, XX)]); }  
# 611
for (iv = 0; iv < 1; iv++) { (((q[X])[Y])[iv]) = ((fq->data)[reverse_addr_rank1_assert(611, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, XY)]); }  
# 612
for (iv = 0; iv < 1; iv++) { (((q[X])[Z])[iv]) = ((fq->data)[reverse_addr_rank1_assert(612, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, XZ)]); }  
# 613
for (iv = 0; iv < 1; iv++) { (((q[Y])[X])[iv]) = (((q[X])[Y])[iv]); }  
# 614
for (iv = 0; iv < 1; iv++) { (((q[Y])[Y])[iv]) = ((fq->data)[reverse_addr_rank1_assert(614, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, YY)]); }  
# 615
for (iv = 0; iv < 1; iv++) { (((q[Y])[Z])[iv]) = ((fq->data)[reverse_addr_rank1_assert(615, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, YZ)]); }  
# 616
...
int ip1[1]; 
# 627
for (iv = 0; iv < 1; iv++) { (im1[iv]) = lees_edw_ic_to_buff(be->le, ic[iv], -1); }  
# 628
for (iv = 0; iv < 1; iv++) { (ip1[iv]) = lees_edw_ic_to_buff(be->le, ic[iv], +1); }  
# 630
for (iv = 0; iv < 1; iv++) { (im1[iv]) = lees_edw_index(be->le, im1[iv], jc[iv], kc[iv]); }  
# 631
for (iv = 0; iv < 1; iv++) { (ip1[iv]) = lees_edw_index(be->le, ip1[iv], jc[iv], kc[iv]); }  
# 633
for (iv = 0; iv < 1; iv++) { 
# 634
if (maskv[iv]) { 
# 635
(((w[X])[X])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(636, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], X)]) - ((hydro->u)[reverse_addr_rank1_assert(637, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], X)]))); 
# 638
}  
# 639
}  
...
if (maskv[iv]) { 
# 642
(((w[Y])[X])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(643, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], Y)]) - ((hydro->u)[reverse_addr_rank1_assert(644, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], Y)]))); 
# 645
}  
# 646
}  
...
if (maskv[iv]) { 
# 649
(((w[Z])[X])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(650, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], Z)]) - ((hydro->u)[reverse_addr_rank1_assert(651, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], Z)]))); 
# 652
}  
# 653
}  
...
for (iv = 0; iv < 1; iv++) { 
# 663
(((w[X])[Y])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(664, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], X)]) - ((hydro->u)[reverse_addr_rank1_assert(665, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], X)]))); 
# 666
}  
# 667
for (iv = 0; iv < 1; iv++) { 
# 668
(((w[Y])[Y])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(669, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], Y)]) - ((hydro->u)[reverse_addr_rank1_assert(670, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], Y)]))); 
# 671
}  
# 672
for (iv = 0; iv < 1; iv++) { 
# 673
(((w[Z])[Y])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(674, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], Z)]) - ((hydro->u)[reverse_addr_rank1_assert(675, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], Z)]))); 
# 676
}  
# 678
for (iv = 0; iv < 1; iv++) { 
...
for (iv = 0; iv < 1; iv++) { 
# 686
(((w[X])[Z])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(687, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], X)]) - ((hydro->u)[reverse_addr_rank1_assert(688, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], X)]))); 
# 689
}  
# 690
for (iv = 0; iv < 1; iv++) { 
# 691
(((w[Y])[Z])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(692, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], Y)]) - ((hydro->u)[reverse_addr_rank1_assert(693, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], Y)]))); 
# 694
}  
# 695
for (iv = 0; iv < 1; iv++) { 
# 696
(((w[Z])[Z])[iv]) = ((0.5) * (((hydro->u)[reverse_addr_rank1_assert(697, "blue_phase_beris_edwards.c", hydro->nsite, 3, ip1[iv], Z)]) - ((hydro->u)[reverse_addr_rank1_assert(698, "blue_phase_beris_edwards.c", hydro->nsite, 3, im1[iv], Z)]))); 
# 699
}  
# 703
for (iv = 0; iv < 1; iv++) { (tr[iv]) = (r3 * (((((w[X])[X])[iv]) + (((w[Y])[Y])[iv])) + (((w[Z])[Z])[iv]))); }  
# 704
for (iv = 0; iv < 1; iv++) { (((w[X])[X])[iv]) -= (tr[iv]); }  
# 705
for (iv = 0; iv < 1; iv++) { (((w[Y])[Y])[iv]) -= (tr[iv]); }  
# 706
for (iv = 0; iv < 1; iv++) { (((w[Z])[Z])[iv]) -= (tr[iv]); }  
# 708
for (iv = 0; iv < 1; iv++) { (trace_qw[iv]) = (0.0); }  
# 710
for (ia = 0; ia < 3; ia++) { 
# 711
for (ib = 0; ib < 3; ib++) { 
# 712
for (iv = 0; iv < 1; iv++) { (trace_qw[iv]) += ((((q[ia])[ib])[iv]) * (((w[ib])[ia])[iv])); }  
# 713
for (iv = 0; iv < 1; iv++) { (((d[ia])[ib])[iv]) = ((0.5) * ((((w[ia])[ib])[iv]) + (((w[ib])[ia])[iv]))); }  
# 714
for (iv = 0; iv < 1; iv++) { (((omega[ia])[ib])[iv]) = ((0.5) * ((((w[ia])[ib])[iv]) - (((w[ib])[ia])[iv]))); }  
# 715
}  
# 716
}  
...
for (iv = 0; iv < 1; iv++) { 
# 721
(((s[ia])[ib])[iv]) = ((((-(2.0)) * ((be->param)->xi)) * ((((q[ia])[ib])[iv]) + (r3 * ((d_[ia])[ib])))) * (trace_qw[iv])); 
# 723
}  
# 724
for (id = 0; id < 3; id++) { 
...
for (iv = 0; iv < 1; iv++) { 
# 726
(((s[ia])[ib])[iv]) += ((((((be->param)->xi) * (((d[ia])[id])[iv])) + (((omega[ia])[id])[iv])) * ((((q[id])[ib])[iv]) + (r3 * ((d_[id])[ib])))) + (((((q[ia])[id])[iv]) + (r3 * ((d_[ia])[id]))) * ((((be->param)->xi) * (((d[id])[ib])[iv])) - (((omega[id])[ib])[iv])))); 
# 731
}  
# 732
}  
...
for (iv = 0; iv < 1; iv++) { 
# 743
noise_reap_n(noise, index + iv, 5, chi); 
# 745
for (id = 0; id < 5; id++) { 
# 746
...
if (maskv[iv]) { 
# 773
(((q[X])[X])[iv]) += (dt * (((((((((((s[X])[X])[iv]) + (((chi_qab[X])[X])[iv])) + (((be->param)->gamma) * ((be->h)[reverse_addr_rank1_assert(776, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, XX)]))) - ((flux->fe)[reverse_addr_rank1_assert(777, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XX)])) + ((flux->fw)[reverse_addr_rank1_assert(778, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XX)])) - ((flux->fy)[reverse_addr_rank1_assert(779, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XX)])) + ((flux->fy)[reverse_addr_rank1_assert(780, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj[iv], XX)])) - ((flux->fz)[reverse_addr_rank1_assert(781, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XX)])) + ((flux->fz)[reverse_addr_rank1_assert(782, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk[iv], XX)]))); 
# 783
}  
# 784
}  
...
if (maskv[iv]) { 
# 788
(((q[X])[Y])[iv]) += (dt * (((((((((((s[X])[Y])[iv]) + (((chi_qab[X])[Y])[iv])) + (((be->param)->gamma) * ((be->h)[reverse_addr_rank1_assert(791, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, XY)]))) - ((flux->fe)[reverse_addr_rank1_assert(792, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XY)])) + ((flux->fw)[reverse_addr_rank1_assert(793, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XY)])) - ((flux->fy)[reverse_addr_rank1_assert(794, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XY)])) + ((flux->fy)[reverse_addr_rank1_assert(795, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj[iv], XY)])) - ((flux->fz)[reverse_addr_rank1_assert(796, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XY)])) + ((flux->fz)[reverse_addr_rank1_assert(797, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk[iv], XY)]))); 
# 798
}  
# 799
}  
...
if (maskv[iv]) { 
# 803
(((q[X])[Z])[iv]) += (dt * (((((((((((s[X])[Z])[iv]) + (((chi_qab[X])[Z])[iv])) + (((be->param)->gamma) * ((be->h)[reverse_addr_rank1_assert(806, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, XZ)]))) - ((flux->fe)[reverse_addr_rank1_assert(807, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XZ)])) + ((flux->fw)[reverse_addr_rank1_assert(808, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XZ)])) - ((flux->fy)[reverse_addr_rank1_assert(809, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XZ)])) + ((flux->fy)[reverse_addr_rank1_assert(810, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj[iv], XZ)])) - ((flux->fz)[reverse_addr_rank1_assert(811, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, XZ)])) + ((flux->fz)[reverse_addr_rank1_assert(812, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk[iv], XZ)]))); 
# 813
}  
# 814
}  
...
if (maskv[iv]) { 
# 818
(((q[Y])[Y])[iv]) += (dt * (((((((((((s[Y])[Y])[iv]) + (((chi_qab[Y])[Y])[iv])) + (((be->param)->gamma) * ((be->h)[reverse_addr_rank1_assert(821, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, YY)]))) - ((flux->fe)[reverse_addr_rank1_assert(822, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, YY)])) + ((flux->fw)[reverse_addr_rank1_assert(823, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, YY)])) - ((flux->fy)[reverse_addr_rank1_assert(824, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, YY)])) + ((flux->fy)[reverse_addr_rank1_assert(825, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj[iv], YY)])) - ((flux->fz)[reverse_addr_rank1_assert(826, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, YY)])) + ((flux->fz)[reverse_addr_rank1_assert(827, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk[iv], YY)]))); 
# 828
}  
# 829
}  
...
if (maskv[iv]) { 
# 833
(((q[Y])[Z])[iv]) += (dt * (((((((((((s[Y])[Z])[iv]) + (((chi_qab[Y])[Z])[iv])) + (((be->param)->gamma) * ((be->h)[reverse_addr_rank1_assert(836, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, YZ)]))) - ((flux->fe)[reverse_addr_rank1_assert(837, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, YZ)])) + ((flux->fw)[reverse_addr_rank1_assert(838, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, YZ)])) - ((flux->fy)[reverse_addr_rank1_assert(839, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, YZ)])) + ((flux->fy)[reverse_addr_rank1_assert(840, "blue_phase_beris_edwards.c", flux->nsite, 5, indexj[iv], YZ)])) - ((flux->fz)[reverse_addr_rank1_assert(841, "blue_phase_beris_edwards.c", flux->nsite, 5, index + iv, YZ)])) + ((flux->fz)[reverse_addr_rank1_assert(842, "blue_phase_beris_edwards.c", flux->nsite, 5, indexk[iv], YZ)]))); 
# 843
}  
# 844
}  
# 846
for (iv = 0; iv < 1; iv++) { ((fq->data)[reverse_addr_rank1_assert(846, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, XX)]) = (((q[X])[X])[iv]); }  
# 847
for (iv = 0; iv < 1; iv++) { ((fq->data)[reverse_addr_rank1_assert(847, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, XY)]) = (((q[X])[Y])[iv]); }  
# 848
for (iv = 0; iv < 1; iv++) { ((fq->data)[reverse_addr_rank1_assert(848, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, XZ)]) = (((q[X])[Z])[iv]); }  
# 849
for (iv = 0; iv < 1; iv++) { ((fq->data)[reverse_addr_rank1_assert(849, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, YY)]) = (((q[Y])[Y])[iv]); }  
# 850
for (iv = 0; iv < 1; iv++) { ((fq->data)[reverse_addr_rank1_assert(850, "blue_phase_beris_edwards.c", fq->nsites, 5, index + iv, YZ)]) = (((q[Y])[Z])[iv]); }  
# 853
}  
# 856
} 
#endif
# 879 "blue_phase_beris_edwards.c"
int beris_edw_tmatrix(double t[3][3][5]) { 
# 881
int ia, ib, id; 
# 882
...
kernel_ctxt_t *ctxt = (__null); 
# 923
fe_t *fe_target = (__null); 
# 925
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 925, __PRETTY_FUNCTION__); 
# 926
(fe) ? static_cast< void>(0) : __assert_fail("fe", "blue_phase_beris_edwards.c", 926, __PRETTY_FUNCTION__); 
# 928
cs_nlocal(be->cs, nlocal); 
# 930
(limits.imin) = 1; (limits.imax) = (nlocal[X]); 
...
# 941
do { (__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : beris_edw_h_kernel_v(ctxt->target, be->target, fe_target); } while (0); 
# 942
; 
# 943
{ tdpErrorHandler(tdpPeekAtLastError(), "blue_phase_beris_edwards.c", 943, 1); } ; 
# 944
{ tdpErrorHandler(tdpDeviceSynchronize(), "blue_phase_beris_edwards.c", 944, 1); } ; 
# 946
...
void beris_edw_h_kernel_v(kernel_ctxt_t *ktx, beris_edw_t *be, fe_t *
# 962
fe) ;
#if 0
# 962
...
# 967
(ktx) ? static_cast< void>(0) : __assert_fail("ktx", "blue_phase_beris_edwards.c", 967, __PRETTY_FUNCTION__); 
# 968
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 968, __PRETTY_FUNCTION__); 
# 969
(fe) ? static_cast< void>(0) : __assert_fail("fe", "blue_phase_beris_edwards.c", 969, __PRETTY_FUNCTION__); 
# 970
((fe->func)->htensor_v) ? static_cast< void>(0) : __assert_fail("fe->func->htensor_v", "blue_phase_beris_edwards.c", 970, __PRETTY_FUNCTION__); 
# 972
...
for (iv = 0; iv < 1; iv++) { 
# 986
((be->h)[reverse_addr_rank1_assert(986, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, XX)]) = (((h[X])[X])[iv]); 
# 987
}  
# 988
for (iv = 0; iv < 1; iv++) { 
# 989
((be->h)[reverse_addr_rank1_assert(989, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, XY)]) = (((h[X])[Y])[iv]); 
# 990
}  
# 991
for (iv = 0; iv < 1; iv++) { 
# 992
((be->h)[reverse_addr_rank1_assert(992, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, XZ)]) = (((h[X])[Z])[iv]); 
# 993
}  
# 994
for (iv = 0; iv < 1; iv++) { 
# 995
((be->h)[reverse_addr_rank1_assert(995, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, YY)]) = (((h[Y])[Y])[iv]); 
# 996
}  
# 997
for (iv = 0; iv < 1; iv++) { 
# 998
((be->h)[reverse_addr_rank1_assert(998, "blue_phase_beris_edwards.c", be->nall, 5, index + iv, YZ)]) = (((h[Y])[Z])[iv]); 
# 999
}  
# 1000
}  
...
#endif
# 1018 "blue_phase_beris_edwards.c"
int beris_edw_fix_swd(beris_edw_t *be, colloids_info_t *cinfo, hydro_t *
# 1019
hydro, map_t *map) { 
# 1021
int nlocal[3]; 
...
kernel_ctxt_t *ctxt = (__null); 
# 1028
(be) ? static_cast< void>(0) : __assert_fail("be", "blue_phase_beris_edwards.c", 1028, __PRETTY_FUNCTION__); 
# 1029
(cinfo) ? static_cast< void>(0) : __assert_fail("cinfo", "blue_phase_beris_edwards.c", 1029, __PRETTY_FUNCTION__); 
# 1030
(map) ? static_cast< void>(0) : __assert_fail("map", "blue_phase_beris_edwards.c", 1030, __PRETTY_FUNCTION__); 
# 1032
...
kernel_ctxt_launch_param(ctxt, &nblk, &ntpb); 
# 1045
do { (__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : beris_edw_fix_swd_kernel(ctxt->target, cinfo->target, hydro->target, map->target, noffset[X], noffset[Y], noffset[Z]); } while (0); 
# 1047
; 
# 1048
{ tdpErrorHandler(tdpPeekAtLastError(), "blue_phase_beris_edwards.c", 1048, 1); } ; 
# 1049
{ tdpErrorHandler(tdpDeviceSynchronize(), "blue_phase_beris_edwards.c", 1049, 1); } ; 
# 1051
kernel_ctxt_free(ctxt); 
# 1053
return 0; 
...
void beris_edw_fix_swd_kernel(kernel_ctxt_t *ktx, colloids_info_t *cinfo, hydro_t *
# 1072
hydro, map_t *map, int noffsetx, int 
# 1073
noffsety, int noffsetz) ;
#if 0
# 1073
{ 
# 1075
...
__attribute__((unused)) int kiterations; 
# 1078
(ktx) ? static_cast< void>(0) : __assert_fail("ktx", "blue_phase_beris_edwards.c", 1078, __PRETTY_FUNCTION__); 
# 1079
(cinfo) ? static_cast< void>(0) : __assert_fail("cinfo", "blue_phase_beris_edwards.c", 1079, __PRETTY_FUNCTION__); 
# 1080
(hydro) ? static_cast< void>(0) : __assert_fail("hydro", "blue_phase_beris_edwards.c", 1080, __PRETTY_FUNCTION__); 
# 1081
(map) ? static_cast< void>(0) : __assert_fail("map", "blue_phase_beris_edwards.c", 1081, __PRETTY_FUNCTION__); 
# 1083
...
for (ia = 0; ia < 3; ia++) { 
# 1104
((hydro->u)[reverse_addr_rank1_assert(1104, "blue_phase_beris_edwards.c", hydro->nsite, 3, index, ia)]) = (0.0); 
# 1105
}  
# 1106
}  
...
for (ia = 0; ia < 3; ia++) { 
# 1132
((hydro->u)[reverse_addr_rank1_assert(1132, "blue_phase_beris_edwards.c", hydro->nsite, 3, index, ia)]) = (u[ia]); 
# 1133
}  
# 1134
}  
...
# 1 "blue_phase_beris_edwards.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__32_blue_phase_beris_edwards_cpp1_ii_b4a6d5f7
# 1 "blue_phase_beris_edwards.cudafe1.stub.c"
#include "blue_phase_beris_edwards.cudafe1.stub.c"
# 1 "blue_phase_beris_edwards.cudafe1.stub.c"
