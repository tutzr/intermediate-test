...
# 32 "advection_bcs.c"
void advection_bcs_no_flux_kernel_v(kernel_ctxt_t * ktx, advflux_t * flux, map_t * map); 
# 44 "advection_bcs.c"
int advection_bcs_no_normal_flux(int nf, advflux_t *flux, map_t *map) { 
# 46
int nlocal[3]; 
...
# 51
(flux) ? static_cast< void>(0) : __assert_fail("flux", "advection_bcs.c", 51, __PRETTY_FUNCTION__); 
# 52
(map) ? static_cast< void>(0) : __assert_fail("map", "advection_bcs.c", 52, __PRETTY_FUNCTION__); 
# 54
...
# 65
do { (__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : advection_bcs_no_flux_kernel_v(ctxt->target, flux->target, map->target); } while (0); 
# 66
; 
# 68
...
# 86 "advection_bcs.c"
void advection_bcs_no_flux_kernel_v(kernel_ctxt_t *ktx, advflux_t *
# 87
flux, map_t *
# 88
map) ;
...
# 114
for (iv = 0; iv < 1; iv++) { (mask[iv]) = (((map->status)[index[iv]]) == (MAP_FLUID)); }  
# 116
for (iv = 0; iv < 1; iv++) { (ix[iv]) = ((ic[iv]) - (maskv[iv])); }  
# 117
kernel_coords_index_v(ktx, ix, jc, kc, index); 
# 118
for (iv = 0; iv < 1; iv++) { (maskw[iv]) = (((map->status)[index[iv]]) == (MAP_FLUID)); }  
# 120
for (iv = 0; iv < 1; iv++) { (ix[iv]) = ((ic[iv]) + (maskv[iv])); }  
# 121
kernel_coords_index_v(ktx, ix, jc, kc, index); 
# 122
for (iv = 0; iv < 1; iv++) { (maske[iv]) = (((map->status)[index[iv]]) == (MAP_FLUID)); }  
# 124
for (iv = 0; iv < 1; iv++) { (ix[iv]) = ((jc[iv]) + (maskv[iv])); }  
# 125
kernel_coords_index_v(ktx, ic, ix, kc, index); 
# 126
for (iv = 0; iv < 1; iv++) { (masky[iv]) = (((map->status)[index[iv]]) == (MAP_FLUID)); }  
# 128
for (iv = 0; iv < 1; iv++) { (ix[iv]) = ((kc[iv]) + (maskv[iv])); }  
# 129
kernel_coords_index_v(ktx, ic, jc, ix, index); 
# 130
for (iv = 0; iv < 1; iv++) { (maskz[iv]) = (((map->status)[index[iv]]) == (MAP_FLUID)); }  
# 132
...
for (iv = 0; iv < 1; iv++) { 
# 136
(index[iv]) = reverse_addr_rank1_assert(136, "advection_bcs.c", flux->nsite, flux->nf, index0 + iv, n); 
# 137
}  
# 138
for (iv = 0; iv < 1; iv++) { ((flux->fw)[index[iv]]) *= ((mask[iv]) * (maskw[iv])); }  
# 139
for (iv = 0; iv < 1; iv++) { ((flux->fe)[index[iv]]) *= ((mask[iv]) * (maske[iv])); }  
# 140
for (iv = 0; iv < 1; iv++) { ((flux->fy)[index[iv]]) *= ((mask[iv]) * (masky[iv])); }  
# 141
for (iv = 0; iv < 1; iv++) { ((flux->fz)[index[iv]]) *= ((mask[iv]) * (maskz[iv])); }  
# 142
}  
# 144
}  
...
double mask[7]; 
# 167
(nf > 0) ? static_cast< void>(0) : __assert_fail("nf > 0", "advection_bcs.c", 167, __PRETTY_FUNCTION__); 
# 168
(flx) ? static_cast< void>(0) : __assert_fail("flx", "advection_bcs.c", 168, __PRETTY_FUNCTION__); 
# 169
(map) ? static_cast< void>(0) : __assert_fail("map", "advection_bcs.c", 169, __PRETTY_FUNCTION__); 
# 171
...
for (n = 0; n < nf; n++) { 
# 189
((flx[reverse_addr_rank1_assert(189, "advection_bcs.c", nsites, nf, index0, n)])[c - 1]) *= ((mask[0]) * (mask[c])); 
# 190
}  
# 191
}  
...
cs_t *cs = (__null); 
# 229
(0) ? static_cast< void>(0) : __assert_fail("0", "advection_bcs.c", 229, __PRETTY_FUNCTION__); 
# 230
(fphi) ? static_cast< void>(0) : __assert_fail("fphi", "advection_bcs.c", 230, __PRETTY_FUNCTION__); 
# 232
field_nf(fphi, &nf); 
# 233
cs_nlocal(cs, nlocal); 
...
# 237
(nf <= 5) ? static_cast< void>(0) : __assert_fail("nf <= 5", "advection_bcs.c", 237, __PRETTY_FUNCTION__); 
# 239
...
# 1 "advection_bcs.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__21_advection_bcs_cpp1_ii_49fe87a6
# 1 "advection_bcs.cudafe1.stub.c"
#include "advection_bcs.cudafe1.stub.c"
# 1 "advection_bcs.cudafe1.stub.c"
