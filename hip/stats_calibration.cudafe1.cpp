...
# 69 "stats_calibration.c"
int stats_ahydro_create(pe_t *pe, cs_t *cs, colloids_info_t *cinfo, hydro_t *
# 70
hydro, map_t *map, stats_ahydro_t **pobj) { 
# 72
int ia; 
# 73
int nctotal; 
...
# 83
stats_ahydro_t *obj = (__null); 
# 85
(pe) ? static_cast< void>(0) : __assert_fail("pe", "stats_calibration.c", 85, __PRETTY_FUNCTION__); 
# 86
(cs) ? static_cast< void>(0) : __assert_fail("cs", "stats_calibration.c", 86, __PRETTY_FUNCTION__); 
# 87
(cinfo) ? static_cast< void>(0) : __assert_fail("cinfo", "stats_calibration.c", 87, __PRETTY_FUNCTION__); 
# 88
(hydro) ? static_cast< void>(0) : __assert_fail("hydro", "stats_calibration.c", 88, __PRETTY_FUNCTION__); 
# 89
(map) ? static_cast< void>(0) : __assert_fail("map", "stats_calibration.c", 89, __PRETTY_FUNCTION__); 
# 90
(pobj) ? static_cast< void>(0) : __assert_fail("pobj", "stats_calibration.c", 90, __PRETTY_FUNCTION__); 
# 92
obj = ((stats_ahydro_t *)calloc(1, sizeof(stats_ahydro_t))); 
# 93
(obj) ? static_cast< void>(0) : __assert_fail("obj", "stats_calibration.c", 93, __PRETTY_FUNCTION__); 
# 94
if (obj == (__null)) { pe_fatal(pe, "calloc(stats_ahydro_t) failed\n"); }  
# 96
(obj->pe) = pe; 
# 97
(obj->cs) = cs; 
...
physics_ref(&phys); 
# 107
((ntotal[X]) == (ntotal[Y])) ? static_cast< void>(0) : __assert_fail("ntotal[X] == ntotal[Y]", "stats_calibration.c", 107, __PRETTY_FUNCTION__); 
# 108
((ntotal[Y]) == (ntotal[Z])) ? static_cast< void>(0) : __assert_fail("ntotal[Y] == ntotal[Z]", "stats_calibration.c", 108, __PRETTY_FUNCTION__); 
# 109
...
(obj->a0) = a; 
# 119
(obj->utarget) = ((eta * (0.05000000000000000278)) / (a * rho)); 
# 120
...
# 124
(obj->nfreq) = ((obj->nstokes) / 50); 
# 125
if ((obj->nfreq) < 1) { (obj->nfreq) = 1; }  
# 126
(obj->nstart) = ((length * length) / eta); 
# 132
(f[X]) = (+(0.5022740837420179849)); 
# 133
(f[Y]) = (-(0.105061333197473003)); 
# 134
(f[Z]) = (-(0.8583023133301489471)); 
# 136
for (ia = 0; ia < 3; ia++) { 
# 137
(f[ia]) *= (obj->ftarget); 
...
++(stat->ndata); 
# 175
hydro_memcpy(stat->hydro, hipMemcpyDeviceToHost); 
# 176
map_memcpy(stat->map, hipMemcpyDeviceToHost); 
# 177
...
MPI_Comm comm; 
# 210
(stat) ? static_cast< void>(0) : __assert_fail("stat", "stats_calibration.c", 210, __PRETTY_FUNCTION__); 
# 212
if ((stat->nstart) < 2147483647) { 
# 214
physics_ref(&phys); 
# 215
physics_eta_shear(phys, &eta); 
...
# 282 "stats_calibration.c"
static double stats_calibration_hasimoto(double a, double len) { 
# 284
double f; 
# 286
f = ((((1.0) - ((2.837000000000000188) * (a / len))) + ((4.190000000000000391) * pow(a / len, 3))) - ((27.39999999999999858) * pow(a / len, 6))); 
# 288
return f; 
# 289
} 
# 305 "stats_calibration.c"
static int stats_ahydro_measure(stats_ahydro_t *stat) { 
# 307
...
MPI_Comm comm; 
# 319
(stat) ? static_cast< void>(0) : __assert_fail("stat", "stats_calibration.c", 319, __PRETTY_FUNCTION__); 
# 321
volume = (0.0); 
# 322
for (ia = 0; ia < 3; ia++) { 
...
# 1 "stats_calibration.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__25_stats_calibration_cpp1_ii_5592a94e
# 1 "stats_calibration.cudafe1.stub.c"
#include "stats_calibration.cudafe1.stub.c"
# 1 "stats_calibration.cudafe1.stub.c"
