...
# 140
{ tdpErrorHandler(hipGetSymbolAddress((void **)(&tmp), &const_param), "brazovskii.c", 140, 1); } ; 
# 141
tdpMemcpy(&((obj->target)->param), &tmp, sizeof(fe_brazovskii_t *), hipMemcpyHostToDevice); 
# 143
{ tdpErrorHandler(hipGetSymbolAddress((void **)(&vt), &fe_braz_dvt), "brazovskii.c", 143, 1); } ; 
# 144
tdpMemcpy(&(((obj->target)->super).func), &vt, sizeof(fe_vt_t *), hipMemcpyHostToDevice); 
# 147
tdpMemcpy(&((obj->target)->phi), &(phi->target), sizeof(field_t *), hipMemcpyHostToDevice); 
# 149
tdpMemcpy(&((obj->target)->dphi), &(dphi->target), sizeof(field_grad_t *), hipMemcpyHostToDevice); 
# 151
}  
# 153
(*p) = obj; 
