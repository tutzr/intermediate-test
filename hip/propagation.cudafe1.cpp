...
# 28 "propagation.c"
int lb_propagation_driver(lb_t * lb); 
# 29
int lb_model_swapf(lb_t * lb); 
# 31
void lb_propagation_kernel(kernel_ctxt_t * ktx, lb_t * lb); 
# 32
void lb_propagation_kernel_novector(kernel_ctxt_t * ktx, lb_t * lb); 
# 34
static cs_param_t coords; 
# 35
static lb_collide_param_t lbp; 
# 45 "propagation.c"
int lb_propagation(lb_t *lb) { 
# 47
(lb) ? static_cast< void>(0) : __assert_fail("lb", "propagation.c", 47, __PRETTY_FUNCTION__); 
# 49
...
# 67
(lb) ? static_cast< void>(0) : __assert_fail("lb", "propagation.c", 67, __PRETTY_FUNCTION__); 
# 69
...
# 77
{ tdpErrorHandler(hipMemcpyToSymbol(&coords, (lb->cs)->param, sizeof(cs_param_t), 0, hipMemcpyHostToDevice), "propagation.c", 78, 1); 
# 77 "propagation.c"
} 
# 78
; 
# 79
{ tdpErrorHandler(hipMemcpyToSymbol(&lbp, lb->param, sizeof(lb_collide_param_t), 0, hipMemcpyHostToDevice), "propagation.c", 81, 1); 
# 79 "propagation.c"
} 
# 81
; 
...
# 88
do { (__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : lb_propagation_kernel(ctxt->target, lb->target); } while (0); 
# 89
; 
# 90
{ tdpErrorHandler(tdpPeekAtLastError(), "propagation.c", 90, 1); } ; 
# 91
{ tdpErrorHandler(tdpDeviceSynchronize(), "propagation.c", 91, 1); } ; 
# 93
...
int kiter; 
# 115
(ktx) ? static_cast< void>(0) : __assert_fail("ktx", "propagation.c", 115, __PRETTY_FUNCTION__); 
# 116
(lb) ? static_cast< void>(0) : __assert_fail("lb", "propagation.c", 116, __PRETTY_FUNCTION__); 
# 118
...
# 142
((lb->fprime)[reverse_addr_rank2_assert(142, "propagation.c", lb->nsite, lb->ndist, NVEL, index, n, p)]) = ((lb->f)[reverse_addr_rank2_assert(143, "propagation.c", lb->nsite, lb->ndist, NVEL, indexp, n, p)]); 
# 144
}  
# 145
}  
...
# 171
(lb) ? static_cast< void>(0) : __assert_fail("lb", "propagation.c", 171, __PRETTY_FUNCTION__); 
# 173
...
for (iv = 0; iv < 1; iv++) { 
# 199
(indexp[iv]) = ((index0 + iv) - ((maskv[iv]) * ((((((lbp.cv)[p])[X]) * ((coords.str)[X])) + ((((lbp.cv)[p])[Y]) * ((coords.str)[Y]))) + ((((lbp.cv)[p])[Z]) * ((coords.str)[Z]))))); 
# 202
}  
# 204
for (iv = 0; iv < 1; iv++) { 
# 205
(fprime[reverse_addr_rank2_assert(205, "propagation.c", lbp.nsite, lbp.ndist, NVEL, index0 + iv, n, p)]) = (f[reverse_addr_rank2_assert(206, "propagation.c", lbp.nsite, lbp.ndist, NVEL, indexp[iv], n, p)]); 
# 207
}  
# 208
}  
...
double *tmp2; 
# 231
(lb) ? static_cast< void>(0) : __assert_fail("lb", "propagation.c", 231, __PRETTY_FUNCTION__); 
# 232
(lb->target) ? static_cast< void>(0) : __assert_fail("lb->target", "propagation.c", 232, __PRETTY_FUNCTION__); 
# 234
...
{ 
# 242
{ tdpErrorHandler(tdpMemcpy(&tmp1, &((lb->target)->f), sizeof(double *), hipMemcpyDeviceToHost), "propagation.c", 243, 1); 
# 242 "propagation.c"
} 
# 243
; 
# 244
{ tdpErrorHandler(tdpMemcpy(&tmp2, &((lb->target)->fprime), sizeof(double *), hipMemcpyDeviceToHost), "propagation.c", 245, 1); 
# 244 "propagation.c"
} 
# 245
; 
# 247
{ tdpErrorHandler(tdpMemcpy(&((lb->target)->f), &tmp2, sizeof(double *), hipMemcpyHostToDevice), "propagation.c", 248, 1); 
# 247 "propagation.c"
} 
# 248
; 
# 249
{ tdpErrorHandler(tdpMemcpy(&((lb->target)->fprime), &tmp1, sizeof(double *), hipMemcpyHostToDevice), "propagation.c", 250, 1); 
# 249 "propagation.c"
} 
# 250
; 
...
# 1 "propagation.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__19_propagation_cpp1_ii_0ab76e1d
# 1 "propagation.cudafe1.stub.c"
#include "propagation.cudafe1.stub.c"
# 1 "propagation.cudafe1.stub.c"
