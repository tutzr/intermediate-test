...
cs_init(cs); 
# 47
if ((1 == 1) && ((DATA_MODEL_SOA) != (DATA_MODEL_AOS))) { 
# 48
...
lb_t *lb = (__null); 
# 89
(pe) ? static_cast< void>(0) : __assert_fail("pe", "test_prop.c", 89, __PRETTY_FUNCTION__); 
# 90
(cs) ? static_cast< void>(0) : __assert_fail("cs", "test_prop.c", 90, __PRETTY_FUNCTION__); 
# 92
lb_create(pe, cs, &lb); 
# 93
(lb) ? static_cast< void>(0) : __assert_fail("lb", "test_prop.c", 93, __PRETTY_FUNCTION__); 
# 95
...
# 143
(fabs(f_actual - ((1.0) * (p + (nd * (NVEL))))) < ((double)(2.2204460492503130808e-16L))) ? static_cast< void>(0) : __assert_fail("fabs(f_actual - 1.0*(p + nd*NVEL)) < double(2.22044604925031308085e-16L)", "test_prop.c", 143, __PRETTY_FUNCTION__); 
# 144
}  
# 145
}  
...
lb_t *lb = (__null); 
# 181
(pe) ? static_cast< void>(0) : __assert_fail("pe", "test_prop.c", 181, __PRETTY_FUNCTION__); 
# 182
(cs) ? static_cast< void>(0) : __assert_fail("cs", "test_prop.c", 182, __PRETTY_FUNCTION__); 
# 184
lb_create(pe, cs, &lb); 
# 185
(lb) ? static_cast< void>(0) : __assert_fail("lb", "test_prop.c", 185, __PRETTY_FUNCTION__); 
# 186
lb_ndist_set(lb, ndist); 
# 187
lb_init(lb); 
...
index = cs_index(cs, ic, jc, kc); 
# 203
f_actual = (((((ltot[Y]) * (ltot[Z])) * ((offset[X]) + ic)) + ((ltot[Z]) * ((offset[Y]) + jc))) + ((offset[Z]) + kc)); 
# 206
for (nd = 0; nd < ndist; nd++) { 
# 207
for (p = 0; p < nvel; p++) { 
...
# 254
(fabs(f_actual - f_expect) < ((double)(2.2204460492503130808e-16L))) ? static_cast< void>(0) : __assert_fail("fabs(f_actual - f_expect) < double(2.22044604925031308085e-16L)", "test_prop.c", 254, __PRETTY_FUNCTION__); 
# 255
}  
# 256
}  
...
# 1 "test_prop.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__17_test_prop_cpp1_ii_dc05e19a
# 1 "test_prop.cudafe1.stub.c"
#include "test_prop.cudafe1.stub.c"
# 1 "test_prop.cudafe1.stub.c"
