...
# 154
do { (__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : do_target_kernel1(ctxt->target, data->target); } while (0); 
# 155
; 
# 156
...
# 159
data_copy(data, hipMemcpyDeviceToHost); 
# 160
do_check(cs, iref, data->idata); 
# 165
data_zero(data); 
# 166
do { (__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : do_target_kernel1r(ctxt->target, data->target); } while (0); 
# 167
; 
# 168
...
data_zero(data); 
# 185
do { (__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : do_target_kernel2(ctxt->target, data->target); } while (0); 
# 186
; 
# 187
...
# 423
{ tdpErrorHandler(tdpMemcpy(&((data->target)->idata), &tmp, sizeof(int *), hipMemcpyHostToDevice), "test_kernel.c", 424, 1); 
# 423 "test_kernel.c"
} 
# 424
; 
# 425
{ tdpErrorHandler(tdpMemcpy(&((data->target)->nsites), &nsites, sizeof(int), hipMemcpyHostToDevice), "test_kernel.c", 426, 1); 
# 425 "test_kernel.c"
} 
# 426
; 
...
{ 
# 451
{ tdpErrorHandler(tdpMemcpy(&tmp, &((data->target)->idata), sizeof(int *), hipMemcpyDeviceToHost), "test_kernel.c", 452, 1); 
# 451 "test_kernel.c"
} 
# 452
; 
...
# 477
data_copy(data, hipMemcpyHostToDevice); 
# 479
return 0; 
# 480
} 
...
nsites = (data->nsites); 
# 501
{ tdpErrorHandler(tdpMemcpy(&tmp, &((data->target)->idata), sizeof(int *), hipMemcpyDeviceToHost), "test_kernel.c", 502, 1); 
# 501 "test_kernel.c"
} 
# 502
; 
# 503
if (flag == (hipMemcpyHostToDevice)) { 
# 504
