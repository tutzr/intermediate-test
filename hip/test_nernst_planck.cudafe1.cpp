...
double rho_i; 
# 103
double rho_b, rho_b_local; 
# 105
int valency[2] = {(+1), (-1)}; 
# 106
double diffusivity[2] = {(0.01000000000000000021), (0.01000000000000000021)}; 
# 108
double eunit = (1.0); 
# 109
double epsilon = (3300.0); 
# 110
double beta = (30000.0); 
# 111
double lb; 
...
double ldebye; 
# 113
double rho_el = (0.001000000000000000021); 
# 114
double yd; 
# 115
double ltot[3]; 
...
MPI_Comm comm; 
# 129
(pe) ? static_cast< void>(0) : __assert_fail("pe", "test_nernst_planck.c", 129, __PRETTY_FUNCTION__); 
# 131
...
# 152
(map) ? static_cast< void>(0) : __assert_fail("map", "test_nernst_planck.c", 152, __PRETTY_FUNCTION__); 
# 154
psi_create(pe, cs, nk, &psi); 
# 155
(psi) ? static_cast< void>(0) : __assert_fail("psi", "test_nernst_planck.c", 155, __PRETTY_FUNCTION__); 
# 157
...
psi_halo_rho(psi); 
# 228
nernst_planck_driver(psi, (fe_t *)fe, __null, map); 
# 230
if ((tstep % 1000) == 0) { 
# 232
pe_info(pe, "%d\n", tstep); 
...
# 263
(tmax == 200) ? static_cast< void>(0) : __assert_fail("tmax == 200", "test_nernst_planck.c", 263, __PRETTY_FUNCTION__); 
# 264
((ntotal[X]) == 64) ? static_cast< void>(0) : __assert_fail("ntotal[X] == 64", "test_nernst_planck.c", 264, __PRETTY_FUNCTION__); 
# 265
(fabs(lb - (0.7234315599999999735)) < ((1.192092896e-07F))) ? static_cast< void>(0) : __assert_fail("fabs(lb - 7.2343156e-01) < 1.19209289550781250000e-7F", "test_nernst_planck.c", 265, __PRETTY_FUNCTION__); 
# 266
(fabs(ldebye - (6.064855399999999896)) < ((1.192092896e-07F))) ? static_cast< void>(0) : __assert_fail("fabs(ldebye - 6.0648554e+00) < 1.19209289550781250000e-7F", "test_nernst_planck.c", 266, __PRETTY_FUNCTION__); 
# 267
(fabs(yd - (5.199757599999999935e-05)) < ((1.192092896e-07F))) ? static_cast< void>(0) : __assert_fail("fabs(yd - 5.1997576e-05) < 1.19209289550781250000e-7F", "test_nernst_planck.c", 267, __PRETTY_FUNCTION__); 
# 269
map_free(map); 
# 270
fe_electro_free(fe); 
...
# 313
(field) ? static_cast< void>(0) : __assert_fail("field", "test_nernst_planck.c", 313, __PRETTY_FUNCTION__); 
# 314
(psifield) ? static_cast< void>(0) : __assert_fail("psifield", "test_nernst_planck.c", 314, __PRETTY_FUNCTION__); 
# 315
(rho0field) ? static_cast< void>(0) : __assert_fail("rho0field", "test_nernst_planck.c", 315, __PRETTY_FUNCTION__); 
# 316
(rho1field) ? static_cast< void>(0) : __assert_fail("rho1field", "test_nernst_planck.c", 316, __PRETTY_FUNCTION__); 
# 317
if (field == (__null)) { pe_fatal(psi->pe, "calloc(field) failed\n"); }  
# 318
if (psifield == (__null)) { pe_fatal(psi->pe, "calloc(psifield) failed\n"); }  
# 319
if (rho0field == (__null)) { pe_fatal(psi->pe, "calloc(rho0field) failed\n"); }  
# 320
if (rho1field == (__null)) { pe_fatal(psi->pe, "calloc(rho1field) failed\n"); }  
# 322
for (ic = 1; ic <= (nlocal[X]); ic++) { 
# 324
index = cs_index(cs, ic, jc, kc); 
...
# 328
MPI_Gather(field, nlocal[X], MPI_DOUBLE, psifield, nlocal[X], MPI_DOUBLE, 0, comm); 
# 331
for (ic = 1; ic <= (nlocal[X]); ic++) { 
# 332
index = cs_index(cs, ic, jc, kc); 
...
# 336
MPI_Gather(field, nlocal[X], MPI_DOUBLE, rho0field, nlocal[X], MPI_DOUBLE, 0, comm); 
# 339
for (ic = 1; ic <= (nlocal[X]); ic++) { 
# 340
index = cs_index(cs, ic, jc, kc); 
...
# 344
MPI_Gather(field, nlocal[X], MPI_DOUBLE, rho1field, nlocal[X], MPI_DOUBLE, 0, comm); 
# 347
...
# 351
if (out == (__null)) { pe_fatal(psi->pe, "Could not open %s\n", filename); }  
# 353
for (ic = 1; ic <= (ntotal[X]); ic++) { 
# 354
fprintf(out, "%d %14.7e %14.7e %14.7e\n", ic, psifield[ic - 1], rho0field[ic - 1], rho1field[ic - 1]); 
# 356
}  
# 357
fclose(out); 
...
# 1 "test_nernst_planck.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__26_test_nernst_planck_cpp1_ii_29487d51
# 1 "test_nernst_planck.cudafe1.stub.c"
#include "test_nernst_planck.cudafe1.stub.c"
# 1 "test_nernst_planck.cudafe1.stub.c"
