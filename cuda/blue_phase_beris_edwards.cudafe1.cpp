...
# 153
{ tdpErrorHandler(cudaGetSymbolAddress((void **)(&tmp), static_param), "blue_phase_beris_edwards.c", 153, 1); } ; 
# 154
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->param), &tmp, sizeof(beris_edw_param_t *), cudaMemcpyHostToDevice), "blue_phase_beris_edwards.c", 155, 1); 
...
# 158
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->le), &letarget, sizeof(lees_edw_t *), cudaMemcpyHostToDevice), "blue_phase_beris_edwards.c", 159, 1); 
# 158 "blue_phase_beris_edwards.c"
} 
# 159
; 
# 160
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->flux), &(flx->target), sizeof(advflux_t *), cudaMemcpyHostToDevice), "blue_phase_beris_edwards.c", 161, 1); 
# 160 "blue_phase_beris_edwards.c"
} 
# 161
; 
# 163
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->nall), &(obj->nall), sizeof(int), cudaMemcpyHostToDevice), "blue_phase_beris_edwards.c", 164, 1); 
...
# 166
{ tdpErrorHandler(tdpMemcpy(&((obj->target)->h), &htmp, sizeof(double *), cudaMemcpyHostToDevice), "blue_phase_beris_edwards.c", 167, 1); 
...
double *htmp; 
# 192
{ tdpErrorHandler(tdpMemcpy(&htmp, &((be->target)->h), sizeof(double *), cudaMemcpyDeviceToHost), "blue_phase_beris_edwards.c", 193, 1); 
...
# 224
{ tdpErrorHandler(cudaMemcpyToSymbol(static_param, be->param, sizeof(beris_edw_param_t), 0, cudaMemcpyHostToDevice), "blue_phase_beris_edwards.c", 225, 1); 
...
# 526
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : beris_edw_kernel_v(ctxt->target, be->target, fq->target, fq_grad->target, hydrotarget, (be->flux)->target, map->target, noisetarget); 
# 528
; 
# 530
...
# 941
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : beris_edw_h_kernel_v(ctxt->target, be->target, fe_target); 
# 942
; 
# 943
...
kernel_ctxt_launch_param(ctxt, &nblk, &ntpb); 
# 1045
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : beris_edw_fix_swd_kernel(ctxt->target, cinfo->target, hydro->target, map->target, noffset[X], noffset[Y], noffset[Z]); 
# 1047
; 
# 1048
