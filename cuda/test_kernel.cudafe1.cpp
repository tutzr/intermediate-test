...
# 38
int do_test_kernel(cs_t * cs, kernel_info_t limits, data_t * data); 
# 39
int do_host_kernel(cs_t * cs, kernel_info_t limits, int * mask, int * isum); 
# 40
int do_check(cs_t * cs, int * iref, int * itarget); 
# 41
int do_test_attributes(pe_t * pe); 
# 43
void do_target_kernel1(kernel_ctxt_t * ktx, data_t * data); 
# 44
void do_target_kernel2(kernel_ctxt_t * ktx, data_t * data); 
# 45
void do_target_kernel1r(kernel_ctxt_t * ktx, data_t * data); 
# 47
int data_create(int nsites, data_t * data); 
# 48
int data_free(data_t * data); 
# 49
int data_zero(data_t * data); 
# 50
int data_copy(data_t * data, tdpMemcpyKind flag); 
# 58
int test_kernel_suite() { 
# 60
int nlocal[3]; 
...
# 121
(cs) ? static_cast< void>(0) : __assert_fail("cs", "test_kernel.c", 121, __PRETTY_FUNCTION__); 
# 125
cs_nsites(cs, &nsites); 
# 133
test_assert_info((nsites % 1) == 0, 133, "test_kernel.c"); 
# 135
isum = 0; 
# 136
iref = ((int *)calloc(nsites, sizeof(int))); 
# 137
(iref) ? static_cast< void>(0) : __assert_fail("iref", "test_kernel.c", 137, __PRETTY_FUNCTION__); 
# 142
kernel_ctxt_create(cs, 1, limits, &ctxt); 
# 143
do_host_kernel(cs, limits, iref, &isum); 
# 145
nexpect = (((((limits.imax) - (limits.imin)) + 1) * (((limits.jmax) - (limits.jmin)) + 1)) * (((limits.kmax) - (limits.kmin)) + 1)); 
# 148
test_assert_info(isum == nexpect, 148, "test_kernel.c"); 
# 152
kernel_ctxt_launch_param(ctxt, &nblk, &ntpb); 
# 154
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : do_target_kernel1(ctxt->target, data->target); 
# 155
; 
# 156
{ tdpErrorHandler(tdpPeekAtLastError(), "test_kernel.c", 156, 1); } ; 
# 157
{ tdpErrorHandler(tdpDeviceSynchronize(), "test_kernel.c", 157, 1); } ; 
# 159
data_copy(data, cudaMemcpyDeviceToHost); 
# 160
do_check(cs, iref, data->idata); 
# 165
data_zero(data); 
# 166
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : do_target_kernel1r(ctxt->target, data->target); 
# 167
; 
# 168
{ tdpErrorHandler(tdpPeekAtLastError(), "test_kernel.c", 168, 1); } ; 
# 169
{ tdpErrorHandler(tdpDeviceSynchronize(), "test_kernel.c", 169, 1); } ; 
# 171
data_copy(data, cudaMemcpyDeviceToHost); 
# 172
do_check(cs, iref, data->idata); 
# 174
test_assert_info(isum == (data->isum), 174, "test_kernel.c"); 
# 176
...
data_zero(data); 
# 185
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : do_target_kernel2(ctxt->target, data->target); 
# 186
; 
# 187
{ tdpErrorHandler(tdpPeekAtLastError(), "test_kernel.c", 187, 1); } ; 
# 188
{ tdpErrorHandler(tdpDeviceSynchronize(), "test_kernel.c", 188, 1); } ; 
# 190
data_copy(data, cudaMemcpyDeviceToHost); 
# 191
do_check(cs, iref, data->idata); 
...
# 208 "test_kernel.c"
int do_host_kernel(cs_t *cs, kernel_info_t limits, int *mask, int *
# 209
isum) { 
# 211
int index; 
...
int nsites; 
# 216
(mask) ? static_cast< void>(0) : __assert_fail("mask", "test_kernel.c", 216, __PRETTY_FUNCTION__); 
# 217
(isum) ? static_cast< void>(0) : __assert_fail("isum", "test_kernel.c", 217, __PRETTY_FUNCTION__); 
# 219
cs_nsites(cs, &nsites); 
# 221
(*isum) = 0; 
...
# 231
test_assert_info((ifail >= 0) && (ifail < nsites), 231, "test_kernel.c"); 
# 233
...
int block_sum; 
# 291
__attribute__((unused)) int psum[128]; 
# 293
(psum[__device_builtin_variable_threadIdx.x]) = 0; 
# 294
kiter = kernel_iterations(ktx); 
# 296
kindex = ((1) * (((__device_builtin_variable_blockIdx.x) * (__device_builtin_variable_blockDim.x)) + (__device_builtin_variable_threadIdx.x))); if (kindex < kiter) { 
...
((data->idata)[mem_addr_rank0(data->nsites, index)]) = index; 
# 307
(psum[__device_builtin_variable_threadIdx.x]) += 1; 
# 308
}  
# 312
...
int kmask[1]; 
# 341
(ktx) ? static_cast< void>(0) : __assert_fail("ktx", "test_kernel.c", 341, __PRETTY_FUNCTION__); 
# 343
...
int nlocal[3]; 
# 371
(iref) ? static_cast< void>(0) : __assert_fail("iref", "test_kernel.c", 371, __PRETTY_FUNCTION__); 
# 372
(itarget) ? static_cast< void>(0) : __assert_fail("itarget", "test_kernel.c", 372, __PRETTY_FUNCTION__); 
# 374
cs_nhalo(cs, &nhalo); 
# 375
cs_nlocal(cs, nlocal); 
...
# 387
(0) ? static_cast< void>(0) : __assert_fail("0", "test_kernel.c", 387, __PRETTY_FUNCTION__); 
# 388
}  
# 390
}  
...
# 411
(data->idata) ? static_cast< void>(0) : __assert_fail("data->idata", "test_kernel.c", 411, __PRETTY_FUNCTION__); 
# 413
...
int *tmp; 
# 420
{ tdpErrorHandler(tdpMalloc((void **)(&(data->target)), sizeof(data_t)), "test_kernel.c", 420, 1); } ; 
# 421
{ tdpErrorHandler(tdpMalloc((void **)(&tmp), nsites * sizeof(int)), "test_kernel.c", 421, 1); } ; 
# 422
{ tdpErrorHandler(tdpMemset(tmp, 0, nsites * sizeof(int)), "test_kernel.c", 422, 1); } ; 
# 423
{ tdpErrorHandler(tdpMemcpy(&((data->target)->idata), &tmp, sizeof(int *), cudaMemcpyHostToDevice), "test_kernel.c", 424, 1); 
# 423 "test_kernel.c"
} 
# 424
; 
# 425
{ tdpErrorHandler(tdpMemcpy(&((data->target)->nsites), &nsites, sizeof(int), cudaMemcpyHostToDevice), "test_kernel.c", 426, 1); 
# 425 "test_kernel.c"
} 
# 426
; 
...
{ 
# 451
{ tdpErrorHandler(tdpMemcpy(&tmp, &((data->target)->idata), sizeof(int *), cudaMemcpyDeviceToHost), "test_kernel.c", 452, 1); 
# 451 "test_kernel.c"
} 
# 452
; 
# 453
{ tdpErrorHandler(tdpFree(tmp), "test_kernel.c", 453, 1); } ; 
# 454
{ tdpErrorHandler(tdpFree(data->target), "test_kernel.c", 454, 1); } ; 
# 455
}  
# 457
return 0; 
...
int n; 
# 470
(data) ? static_cast< void>(0) : __assert_fail("data", "test_kernel.c", 470, __PRETTY_FUNCTION__); 
# 472
(data->isum) = 0; 
# 473
...
# 477
data_copy(data, cudaMemcpyHostToDevice); 
# 479
return 0; 
# 480
} 
...
nsites = (data->nsites); 
# 501
{ tdpErrorHandler(tdpMemcpy(&tmp, &((data->target)->idata), sizeof(int *), cudaMemcpyDeviceToHost), "test_kernel.c", 502, 1); 
# 501 "test_kernel.c"
} 
# 502
; 
...
# 504
{ tdpErrorHandler(tdpMemcpy(&((data->target)->nsites), &nsites, sizeof(int), flag), "test_kernel.c", 504, 1); } ; 
# 505
{ tdpErrorHandler(tdpMemcpy(&((data->target)->isum), &(data->isum), sizeof(int), flag), "test_kernel.c", 505, 1); } ; 
# 506
{ tdpErrorHandler(tdpMemcpy(tmp, data->idata, nsites * sizeof(int), flag), "test_kernel.c", 506, 1); } ; 
# 507
}  
# 508
if (flag == (cudaMemcpyDeviceToHost)) { 
# 509
{ tdpErrorHandler(tdpMemcpy(&(data->isum), &((data->target)->isum), sizeof(int), flag), "test_kernel.c", 509, 1); } ; 
# 510
{ tdpErrorHandler(tdpMemcpy(data->idata, tmp, nsites * sizeof(int), flag), "test_kernel.c", 510, 1); } ; 
# 511
}  
# 512
}  
...
int value; 
# 529
(pe) ? static_cast< void>(0) : __assert_fail("pe", "test_kernel.c", 529, __PRETTY_FUNCTION__); 
# 531
tdpGetDeviceCount(&ndevice); 
# 533
if (ndevice > 0) { 
# 534
{ tdpErrorHandler(tdpGetDevice(&device), "test_kernel.c", 534, 1); } ; 
# 535
{ tdpErrorHandler(tdpDeviceGetAttribute(&value, cudaDevAttrManagedMemory, device), "test_kernel.c", 535, 1); } ; 
# 536
pe_info(pe, "Device:                 %d\n", device); 
# 537
...
# 1 "test_kernel.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__19_test_kernel_cpp1_ii_bb3ec4e5
# 1 "test_kernel.cudafe1.stub.c"
#include "test_kernel.cudafe1.stub.c"
# 1 "test_kernel.cudafe1.stub.c"
