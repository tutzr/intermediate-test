...
# 147
{ tdpErrorHandler(cudaGetSymbolAddress((void **)(&fwd), fs), "phi_force_colloid.c", 147, 1); } ; 
# 148
tdpMemcpy(fwd, fw, (3) * sizeof(double), cudaMemcpyHostToDevice); 
# 150
TIMER_start(TIMER_PHI_FORCE_CALC); 
# 152
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : pth_force_map_kernel(ctxt->target, pth->target, hydro->target, map->target); 
# 153
; 
# 155
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : pth_force_wall_kernel(ctxt->target, pth->target, map->target, wallt, fwd); 
# 156
; 
# 157
...
# 161
tdpMemcpy(fw, fwd, (3) * sizeof(double), cudaMemcpyDeviceToHost); 
# 162
...
kernel_ctxt_launch_param(ctxt, &nblk, &ntpb); 
# 249
{ tdpErrorHandler(cudaGetSymbolAddress((void **)(&fwd), fs), "phi_force_colloid.c", 249, 1); } ; 
# 250
tdpMemcpy(fwd, fw, (3) * sizeof(double), cudaMemcpyHostToDevice); 
# 252
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : pth_force_map_kernel(ctxt->target, pth->target, hydro->target, map->target); 
...
TIMER_start(TIMER_PHI_FORCE_CALC); 
# 297
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : pth_force_fluid_kernel_v(ctxt->target, pth->target, hydro->target); 
# 298
; 
# 299
tdpDeviceSynchronize(); 
