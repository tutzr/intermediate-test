...
# 106
{ tdpErrorHandler(cudaGetSymbolAddress((void **)(&tmp), static_param), "wall.c", 106, 1); } ; 
# 107
tdpMemcpy(&((wall->target)->param), &tmp, sizeof(wall_param_t *), cudaMemcpyHostToDevice); 
# 109
}  
# 111
(*p) = wall; 
...
int *tmp; 
# 128
tdpMemcpy(&tmp, &((wall->target)->linki), sizeof(int *), cudaMemcpyDeviceToHost); 
# 130
tdpFree(tmp); 
# 131
tdpMemcpy(&tmp, &((wall->target)->linkj), sizeof(int *), cudaMemcpyDeviceToHost); 
# 133
tdpFree(tmp); 
# 134
tdpMemcpy(&tmp, &((wall->target)->linkp), sizeof(int *), cudaMemcpyDeviceToHost); 
# 136
tdpFree(tmp); 
# 137
tdpMemcpy(&tmp, &((wall->target)->linku), sizeof(int *), cudaMemcpyDeviceToHost); 
# 139
tdpFree(tmp); 
# 140
tdpFree(wall->target); 
...
# 322
tdpMemcpy(&((wall->target)->linki), &tmp, sizeof(int *), cudaMemcpyHostToDevice); 
# 324
tdpMalloc((void **)(&tmp), (wall->nlink) * sizeof(int)); 
# 325
tdpMemcpy(&((wall->target)->linkj), &tmp, sizeof(int *), cudaMemcpyHostToDevice); 
# 327
tdpMalloc((void **)(&tmp), (wall->nlink) * sizeof(int)); 
# 328
tdpMemcpy(&((wall->target)->linkp), &tmp, sizeof(int *), cudaMemcpyHostToDevice); 
# 330
tdpMalloc((void **)(&tmp), (wall->nlink) * sizeof(int)); 
# 331
tdpMemcpy(&((wall->target)->linku), &tmp, sizeof(int *), cudaMemcpyHostToDevice); 
# 333
}  
# 334
}  
...
# 494
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : wall_setu_kernel(wall->target, (wall->lb)->target); 
# 495
; 
# 497
...
if ((wall->nlink) == 0) { return 0; }  
# 552
{ tdpErrorHandler(cudaMemcpyToSymbol(static_param, wall->param, sizeof(wall_param_t), 0, cudaMemcpyHostToDevice), "wall.c", 553, 1); 
# 552 "wall.c"
} 
# 553
; 
...
kernel_launch_param(wall->nlink, &nblk, &ntpb); 
# 557
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : wall_bbl_kernel(wall->target, (wall->lb)->target, (wall->map)->target); 
# 558
; 
# 560
...
if (ndevice > 0) { 
# 790
tdpMemcpy(ftmp, (wall->target)->fnet, (3) * sizeof(double), cudaMemcpyDeviceToHost); 
# 792
((wall->fnet)[X]) += (ftmp[X]); 
# 793
((wall->fnet)[Y]) += (ftmp[Y]); 
...
# 796
tdpMemcpy((wall->target)->fnet, ftmp, (3) * sizeof(double), cudaMemcpyHostToDevice); 
# 798
}  
# 802
(f[X]) = ((wall->fnet)[X]); 
