...
# 77
tdpMemcpy(&((obj->target)->nsites), &(obj->nsites), sizeof(int), cudaMemcpyHostToDevice); 
# 80
if (method == (PTH_METHOD_DIVERGENCE)) { 
# 81
tdpMalloc((void **)(&tmp), ((3 * 3) * (obj->nsites)) * sizeof(double)); 
# 82
tdpMemcpy(&((obj->target)->str), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 84
}  
# 85
}  
...
if (ndevice > 0) { 
# 108
tdpMemcpy(&tmp, &((pth->target)->str), sizeof(double *), cudaMemcpyDeviceToHost); 
# 110
if (tmp) { tdpFree(tmp); }  
# 111
tdpFree(pth->target); 
...
# 199
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : pth_kernel_a_v(ctxt->target, pth->target, fe_target); 
# 200
; 
# 201
}  
...
{ 
# 205
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : pth_kernel_v(ctxt->target, pth->target, fe_target); 
# 206
; 
# 207
}  
