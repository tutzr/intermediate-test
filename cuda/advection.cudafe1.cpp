...
# 188
tdpMemcpy(&((obj->target)->fe), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 190
tdpMalloc((void **)(&tmp), nsz); 
# 191
tdpMemcpy(&((obj->target)->fw), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 193
tdpMalloc((void **)(&tmp), nsz); 
# 194
tdpMemcpy(&((obj->target)->fy), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 196
tdpMalloc((void **)(&tmp), nsz); 
# 197
tdpMemcpy(&((obj->target)->fz), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 199
tdpMemcpy(&((obj->target)->nf), &(obj->nf), sizeof(int), cudaMemcpyHostToDevice); 
# 200
tdpMemcpy(&((obj->target)->nsite), &(obj->nsite), sizeof(int), cudaMemcpyHostToDevice); 
# 203
if (cs) { cs_target(cs, &cstarget); }  
# 204
if (le) { lees_edw_target(le, &letarget); }  
# 205
tdpMemcpy(&((obj->target)->cs), &cstarget, sizeof(cs_t *), cudaMemcpyHostToDevice); 
# 207
tdpMemcpy(&((obj->target)->le), &letarget, sizeof(lees_edw_t *), cudaMemcpyHostToDevice); 
# 209
}  
# 211
(*pobj) = obj; 
...
if (ndevice > 0) { 
# 232
tdpMemcpy(&tmp, &((obj->target)->fe), sizeof(double *), cudaMemcpyDeviceToHost); 
# 233
tdpFree(tmp); 
# 234
tdpMemcpy(&tmp, &((obj->target)->fw), sizeof(double *), cudaMemcpyDeviceToHost); 
# 235
tdpFree(tmp); 
# 236
tdpMemcpy(&tmp, &((obj->target)->fy), sizeof(double *), cudaMemcpyDeviceToHost); 
# 237
tdpFree(tmp); 
# 238
tdpMemcpy(&tmp, &((obj->target)->fz), sizeof(double *), cudaMemcpyDeviceToHost); 
# 239
tdpFree(tmp); 
# 240
tdpFree(obj->target); 
...
# 348
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : advection_le_1st_kernel(ctxt->target, flux->target, hydro->target, field->target); 
# 349
; 
# 351
...
kernel_ctxt_launch_param(ctxt, &nblk, &ntpb); 
# 507
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : advection_2nd_kernel_v(ctxt->target, flux->target, hydro->target, field->target); 
# 508
; 
# 510
tdpDeviceSynchronize(); 
...
# 776
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : advection_le_3rd_kernel_v(ctxt->target, letarget, flux->target, hydro->target, field->target); 
# 778
; 
# 779
} else 
...
{ 
# 781
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : advection_3rd_kernel_v(ctxt->target, flux->target, hydro->target, field->target); 
# 782
; 
# 783
}  
