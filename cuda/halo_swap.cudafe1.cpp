...
# 210
tdpMemcpy(&((halo->target)->fxlo), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 212
tdpMalloc((void **)(&tmp), sz); 
# 213
tdpMemcpy(&((halo->target)->fxhi), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 216
tdpMalloc((void **)(&tmp), sz); 
# 217
tdpMemcpy(&((halo->target)->hxlo), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 219
tdpMalloc((void **)(&tmp), sz); 
# 220
tdpMemcpy(&((halo->target)->hxhi), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 223
sz = ((((((halo->param)->hsz)[Y]) * na) * nb) * sizeof(double)); 
# 225
tdpMalloc((void **)(&tmp), sz); 
# 226
tdpMemcpy(&((halo->target)->fylo), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 228
tdpMalloc((void **)(&tmp), sz); 
# 229
tdpMemcpy(&((halo->target)->fyhi), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 232
tdpMalloc((void **)(&tmp), sz); 
# 233
tdpMemcpy(&((halo->target)->hylo), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 235
tdpMalloc((void **)(&tmp), sz); 
# 236
tdpMemcpy(&((halo->target)->hyhi), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 239
sz = ((((((halo->param)->hsz)[Z]) * na) * nb) * sizeof(double)); 
# 241
tdpMalloc((void **)(&tmp), sz); 
# 242
tdpMemcpy(&((halo->target)->fzlo), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 244
tdpMalloc((void **)(&tmp), sz); 
# 245
tdpMemcpy(&((halo->target)->fzhi), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 248
tdpMalloc((void **)(&tmp), sz); 
# 249
tdpMemcpy(&((halo->target)->hzlo), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 251
tdpMalloc((void **)(&tmp), sz); 
# 252
tdpMemcpy(&((halo->target)->hzhi), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 255
{ tdpErrorHandler(cudaGetSymbolAddress((void **)(&tmpp), const_param), "halo_swap.c", 255, 1); } ; 
# 256
tdpMemcpy(&((halo->target)->param), &tmpp, sizeof(halo_swap_param_t *), cudaMemcpyHostToDevice); 
# 260
...
double *tmp; 
# 285
tdpMemcpy(&tmp, &((halo->target)->fxlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 287
tdpFree(tmp); 
# 288
tdpMemcpy(&tmp, &((halo->target)->fxhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 290
tdpFree(tmp); 
# 291
tdpMemcpy(&tmp, &((halo->target)->fylo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 293
tdpFree(tmp); 
# 294
tdpMemcpy(&tmp, &((halo->target)->fyhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 296
tdpFree(tmp); 
# 297
tdpMemcpy(&tmp, &((halo->target)->fzlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 299
tdpFree(tmp); 
# 300
tdpMemcpy(&tmp, &((halo->target)->fzhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 302
tdpFree(tmp); 
# 304
tdpMemcpy(&tmp, &((halo->target)->hxlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 306
tdpFree(tmp); 
# 307
tdpMemcpy(&tmp, &((halo->target)->hxhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 309
tdpFree(tmp); 
# 310
tdpMemcpy(&tmp, &((halo->target)->hylo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 312
tdpFree(tmp); 
# 313
tdpMemcpy(&tmp, &((halo->target)->hyhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 315
tdpFree(tmp); 
# 316
tdpMemcpy(&tmp, &((halo->target)->hzlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 318
tdpFree(tmp); 
# 319
tdpMemcpy(&tmp, &((halo->target)->hzhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 321
tdpFree(tmp); 
# 323
tdpFree(halo->target); 
...
# 377
{ tdpErrorHandler(cudaMemcpyToSymbol(const_param, halo->param, sizeof(halo_swap_param_t), 0, cudaMemcpyHostToDevice), "halo_swap.c", 378, 1); 
# 377 "halo_swap.c"
} 
# 378
; 
...
# 781
(__cudaPushCallConfiguration(nblk, ntpb, 0, (halo->stream)[X])) ? (void)0 : (halo->data_pack)(halo->target, X, data); 
# 782
; 
# 784
if (ndevice > 0) { 
...
tdpMemcpy(&tmp, &((halo->target)->fxlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 788
tdpMemcpyAsync(halo->fxlo, tmp, ncount * sizeof(double), cudaMemcpyDeviceToHost, (halo->stream)[X]); 
# 790
tdpMemcpy(&tmp, &((halo->target)->fxhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 792
tdpMemcpyAsync(halo->fxhi, tmp, ncount * sizeof(double), cudaMemcpyDeviceToHost, (halo->stream)[X]); 
# 794
}  
# 798
kernel_launch_param(hsz[Y], &nblk, &ntpb); 
# 799
(__cudaPushCallConfiguration(nblk, ntpb, 0, (halo->stream)[Y])) ? (void)0 : (halo->data_pack)(halo->target, Y, data); 
# 800
; 
# 802
if (ndevice > 0) { 
...
tdpMemcpy(&tmp, &((halo->target)->fylo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 806
tdpMemcpyAsync(halo->fylo, tmp, ncount * sizeof(double), cudaMemcpyDeviceToHost, (halo->stream)[Y]); 
# 808
tdpMemcpy(&tmp, &((halo->target)->fyhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 810
tdpMemcpyAsync(halo->fyhi, tmp, ncount * sizeof(double), cudaMemcpyDeviceToHost, (halo->stream)[Y]); 
# 812
}  
# 816
kernel_launch_param(hsz[Z], &nblk, &ntpb); 
# 817
(__cudaPushCallConfiguration(nblk, ntpb, 0, (halo->stream)[Z])) ? (void)0 : (halo->data_pack)(halo->target, Z, data); 
# 818
; 
# 820
if (ndevice > 0) { 
...
tdpMemcpy(&tmp, &((halo->target)->fzlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 824
tdpMemcpyAsync(halo->fzlo, tmp, ncount * sizeof(double), cudaMemcpyDeviceToHost, (halo->stream)[Z]); 
# 826
tdpMemcpy(&tmp, &((halo->target)->fzhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 828
tdpMemcpyAsync(halo->fzhi, tmp, ncount * sizeof(double), cudaMemcpyDeviceToHost, (halo->stream)[Z]); 
# 830
}  
# 835
...
tdpMemcpy(&tmp, &((halo->target)->hxlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 844
tdpMemcpyAsync(tmp, halo->fxhi, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[X]); 
# 847
memcpy(halo->hxhi, halo->fxlo, ncount * sizeof(double)); 
# 848
tdpMemcpy(&tmp, &((halo->target)->hxhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 850
tdpMemcpyAsync(tmp, halo->fxlo, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[X]); 
# 852
} else 
# 853
{ 
...
tdpMemcpy(&tmp, &((halo->target)->hxlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 864
tdpMemcpyAsync(tmp, halo->hxlo, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[X]); 
# 866
}  
# 867
if ((mc == 1) && (ndevice > 0)) { 
...
tdpMemcpy(&tmp, &((halo->target)->hxhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 870
tdpMemcpyAsync(tmp, halo->hxhi, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[X]); 
# 872
}  
# 873
}  
...
kernel_launch_param(hsz[X], &nblk, &ntpb); 
# 877
(__cudaPushCallConfiguration(nblk, ntpb, 0, (halo->stream)[X])) ? (void)0 : (halo->data_unpack)(halo->target, X, data); 
# 878
; 
# 883
...
tdpMemcpy(&tmp, &((halo->target)->hylo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 918
tdpMemcpyAsync(tmp, halo->fyhi, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[Y]); 
# 921
memcpy(halo->hyhi, halo->fylo, ncount * sizeof(double)); 
# 922
tdpMemcpy(&tmp, &((halo->target)->hyhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 924
tdpMemcpyAsync(tmp, halo->fylo, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[Y]); 
# 926
} else 
# 927
{ 
...
tdpMemcpy(&tmp, &((halo->target)->hylo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 938
tdpMemcpyAsync(tmp, halo->hylo, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[Y]); 
# 940
}  
# 941
if ((mc == 1) && (ndevice > 0)) { 
...
tdpMemcpy(&tmp, &((halo->target)->hyhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 944
tdpMemcpyAsync(tmp, halo->hyhi, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[Y]); 
# 946
}  
# 947
}  
...
kernel_launch_param(hsz[Y], &nblk, &ntpb); 
# 952
(__cudaPushCallConfiguration(nblk, ntpb, 0, (halo->stream)[Y])) ? (void)0 : (halo->data_unpack)(halo->target, Y, data); 
# 953
; 
# 958
...
tdpMemcpy(&tmp, &((halo->target)->hzlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 1015
tdpMemcpyAsync(tmp, halo->fzhi, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[Z]); 
# 1018
tdpMemcpy(&tmp, &((halo->target)->hzhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 1020
tdpMemcpyAsync(tmp, halo->fzlo, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[Z]); 
# 1022
} else 
# 1023
{ 
...
tdpMemcpy(&tmp, &((halo->target)->hzlo), sizeof(double *), cudaMemcpyDeviceToHost); 
# 1034
tdpMemcpyAsync(tmp, halo->hzlo, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[Z]); 
# 1036
}  
# 1037
if ((mc == 1) && (ndevice > 0)) { 
...
tdpMemcpy(&tmp, &((halo->target)->hzhi), sizeof(double *), cudaMemcpyDeviceToHost); 
# 1040
tdpMemcpyAsync(tmp, halo->hzhi, ncount * sizeof(double), cudaMemcpyHostToDevice, (halo->stream)[Z]); 
# 1042
}  
# 1043
}  
...
kernel_launch_param(hsz[Z], &nblk, &ntpb); 
# 1047
(__cudaPushCallConfiguration(nblk, ntpb, 0, (halo->stream)[Z])) ? (void)0 : (halo->data_unpack)(halo->target, Z, data); 
# 1048
; 
# 1050
tdpStreamSynchronize((halo->stream)[X]); 
