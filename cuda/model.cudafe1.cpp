...
if (ndevice > 0) { 
# 122
tdpMemcpy(&tmp, &((lb->target)->f), sizeof(double *), cudaMemcpyDeviceToHost); 
# 123
tdpFree(tmp); 
# 125
tdpMemcpy(&tmp, &((lb->target)->fprime), sizeof(double *), cudaMemcpyDeviceToHost); 
# 127
tdpFree(tmp); 
# 128
tdpFree(lb->target); 
...
# 185
tdpMemcpy(&tmpf, &((lb->target)->f), sizeof(double *), cudaMemcpyDeviceToHost); 
# 187
switch (flag) { 
# 188
...
# 269
tdpMemcpy(&((lb->target)->f), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 271
tdpMalloc((void **)(&tmp), ndata * sizeof(double)); 
# 272
tdpMemset(tmp, 0, ndata * sizeof(double)); 
# 273
tdpMemcpy(&((lb->target)->fprime), &tmp, sizeof(double *), cudaMemcpyHostToDevice); 
# 276
{ tdpErrorHandler(cudaGetSymbolAddress((void **)(&ptmp), static_param), "model.c", 276, 1); } ; 
# 277
tdpMemcpy(&((lb->target)->param), &ptmp, sizeof(lb_collide_param_t *), cudaMemcpyHostToDevice); 
# 279
}  
# 287
...
# 320
{ tdpErrorHandler(cudaMemcpyToSymbol(static_param, lb->param, sizeof(lb_collide_param_t), 0, cudaMemcpyHostToDevice), "model.c", 321, 1); 
# 320 "model.c"
} 
# 321
; 
...
case LB_HALO_TARGET:  
# 811
tdpMemcpy(&data, &((lb->target)->f), sizeof(double *), cudaMemcpyDeviceToHost); 
# 812
