...
# 253
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : phi_ch_flux_mu1_kernel(ctxt->target, letarget, fetarget, (pch->flux)->target, mobility); 
# 254
; 
# 255
...
kernel_ctxt_launch_param(ctxt, &nblk, &ntpb); 
# 921
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : phi_ch_ufs_kernel(ctxt->target, le, phif->target, (pch->flux)->target, ys, wz); 
# 922
; 
# 924
...
kernel_ctxt_launch_param(ctxt, &nblk, &ntpb); 
# 1014
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : phi_ch_flux_mu_ext_kernel(ctxt->target, letarget, (pch->flux)->target, ch); 
# 1015
; 
# 1016
