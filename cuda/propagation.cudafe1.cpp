...
# 77
{ tdpErrorHandler(cudaMemcpyToSymbol(coords, (lb->cs)->param, sizeof(cs_param_t), 0, cudaMemcpyHostToDevice), "propagation.c", 78, 1); 
# 77 "propagation.c"
} 
# 78
; 
# 79
{ tdpErrorHandler(cudaMemcpyToSymbol(lbp, lb->param, sizeof(lb_collide_param_t), 0, cudaMemcpyHostToDevice), "propagation.c", 81, 1); 
# 79 "propagation.c"
} 
# 81
; 
...
# 88
(__cudaPushCallConfiguration(nblk, ntpb, 0, 0)) ? (void)0 : lb_propagation_kernel(ctxt->target, lb->target); 
# 89
; 
# 90
...
{ 
# 242
{ tdpErrorHandler(tdpMemcpy(&tmp1, &((lb->target)->f), sizeof(double *), cudaMemcpyDeviceToHost), "propagation.c", 243, 1); 
# 242 "propagation.c"
} 
# 243
; 
# 244
{ tdpErrorHandler(tdpMemcpy(&tmp2, &((lb->target)->fprime), sizeof(double *), cudaMemcpyDeviceToHost), "propagation.c", 245, 1); 
# 244 "propagation.c"
} 
# 245
; 
# 247
{ tdpErrorHandler(tdpMemcpy(&((lb->target)->f), &tmp2, sizeof(double *), cudaMemcpyHostToDevice), "propagation.c", 248, 1); 
# 247 "propagation.c"
} 
# 248
; 
# 249
{ tdpErrorHandler(tdpMemcpy(&((lb->target)->fprime), &tmp1, sizeof(double *), cudaMemcpyHostToDevice), "propagation.c", 250, 1); 
# 249 "propagation.c"
} 
# 250
; 
